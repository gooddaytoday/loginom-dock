...
const cwds = [root, ...manifest.tasks.map(x=>x.cwd)];
function checkedHooks(result, trusted = false) {
  ...
  for (const entry of result.data) {
    ...
    if(hooks.length!==5) throw new Error('Canonical project hooks were not discovered exactly five times');
    for(const [event,groups] of Object.entries(expected.hooks)) {
      const h=hooks.find(x=>x.eventName===eventNames[event]); const definition=groups[0].hooks[0];
  ...
  if(keys.size!==5) throw new Error('Worktrees do not share one canonical hook identity');
...
try {
 ...
 if(process.argv.includes('--trust')) {
   ...
   writeFileSync(join(rollout,'hooks-trust-verified.json'),JSON.stringify(receipt,null,2)+'\n',{mode:0o600});
   process.stdout.write(JSON.stringify({generation,trusted_hooks:receipt.trusted_hooks,runtimeReloaded:false,
     ...
     receipt:join(rollout,'hooks-trust-verified.json')})+'\n');
