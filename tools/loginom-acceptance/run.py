...
def validate_inputs(args):
    ...
    if getattr(args,'goal',None) in ('replacement-node-complete','join-node-complete','join-review-complete','union-node-complete','union-review-complete'):
        (replacement_upload_probe if args.goal=='replacement-node-complete' else join_upload_probe).validate_catalog(args.manifest_uri,args.manifest_sha256,args.storage_directory)
    if args.run:
...
def execute(args):
    ...
    join_probe=replacement_upload_probe if goal_id=="replacement-node-complete" else union_review_upload_probe if goal_id=="union-review-complete" else union_upload_probe if goal_id=="union-node-complete" else join_review_upload_probe if goal_id=="join-review-complete" else join_upload_probe
    join_fixture_dir="fixtures/replacement" if goal_id=="replacement-node-complete" else "fixtures/union-review" if goal_id=="union-review-complete" else "fixtures/union" if goal_id=="union-node-complete" else "fixtures/join-review" if goal_id=="join-review-complete" else "fixtures/join"
    ...
    if goal_id in ('replacement-node-complete','join-node-complete','join-review-complete','union-node-complete','union-review-complete'):
    ...
    try:
        ...
        if args.goal in ('replacement-node-complete','union-review-complete','union-node-complete','join-review-complete','join-node-complete','node-apply-complete','calculator-node-complete','grouping-node-complete','sales-sorting-complete','reform-node-complete','filter-node-complete'):
...
def main():
    ...
    parser.add_argument("--goal", choices=["replacement-node-complete", "union-review-complete", "union-node-complete", "join-review-complete", "join-node-complete", "prepare-workspace", "basic-graph", "auto-link-retain", "auto-link-remove", "palette-inventory", "checkbox-roundtrip", "context-menu-checkbox", "root-checkbox", "file-storage-inspect", "file-upload-probe", "file-upload-verify", "data-pipeline", "import-roundtrip", "calculator-roundtrip", "node-import-roundtrip", "node-apply-complete", "calculator-node-complete", "grouping-node-complete", "sales-sorting-complete", "reform-node-complete", "filter-node-complete"], default="basic-graph")
