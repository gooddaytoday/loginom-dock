...
def verify_persistence(events, before_request, after_request, expected, graph, path, preparation, save_ids, *, revisions):
    ...
    try:
        ...
        for op, key, reopened in zip(save_ids, ('package.save_checkpoint', 'package.save_as'), (False, True)):
            ...
            previous = ei
            params = dict(path=path, conflict_policy='replace')
            ...
            cp = start['checkpoint']
            if cp != end.get('checkpoint') or cp.get('path') != path or cp.get('graph') != graph or cp.get('package_identity', {}).get('path') != path: failures.append('save_before_graph')
            ...
            trace = result['trace']
            names = ['save_requested', 'saved_package_closed', 'package_open_command_ready', 'reopened_package_observed', 'postcondition_verified'] if reopened else ['save_requested', 'save_flow_completed', 'open_saved_package_observed', 'postcondition_verified']
            selected = []
            for name in names:
            ...
            if conflicts or overwrites:
                if len(conflicts) != 1 or len(overwrites) != 1 or conflicts[0][1].get('path') != path or not selected[0][0] < conflicts[0][0] < overwrites[0] < selected[-2][0]: failures.append('save_overwrite_binding')
