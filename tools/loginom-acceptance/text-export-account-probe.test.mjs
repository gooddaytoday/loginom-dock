...
test('one native open, account-only result, one close before baseline',async()=>{const f=await fixture();assert.equal(f.clicks,1);assert.equal(f.escapes,1);assert.equal(validateAccountEvidence(f.value,f.b),true);assert.equal(f.value.account,'test-2');});
test('hidden/unknown account, changed origin/document/session/workflow and duplicate owner fail',async()=>{
 for(const change of [q=>q[1].line_visible=false,q=>q[1].account='foreign',q=>q[1].origin='https://foreign.invalid',q=>q[1].document_id='other',q=>q[1].session_id='other',q=>q[1].workflow_id='other',q=>q[1].avatar_count=2,q=>q[1].menu_count=2,q=>q[0].menu_count=1]){const f=await fixture(change);assert.equal(f.value.status,'FAILED');assert.throws(()=>validateAccountEvidence(f.value,f.b));assert.ok(f.clicks<=1);}
});
