...
// Independent diagnostic paging of a visible row block; the source reader remains unchanged.
// Every horizontal slice preserves table, schema, segment and native row identity.
export async function readAuditRowPages(channel,table,{rowOffset=0,sampleRows=1,revealRow}={}) {
  requireValue(Number.isInteger(rowOffset)&&rowOffset>=0&&rowOffset<=1000,'Audit row offset outside bound');
  ...
  for(let reads=0;reads<230;reads++) {
    const s=await channel.observe({condition:'Table output columns at '+offset,tablePage:{table,page:{row_offset:rowOffset,row_limit:sampleRows,column_offset:offset,column_limit:limit}},
      ready:s=>s.node_table?.verified===true||['cell_not_visible','row_not_rendered'].includes(s.node_table?.reason)});
    const t=s.node_table;
    if(t.verified!==true&&(t.reason==='row_not_rendered'||t.reason==='cell_not_visible'&&t.horizontal_window?.row_visible===false)){
      requireValue(typeof revealRow==='function','Vertical audit reveal unavailable');
      await revealRow(t.row_index??rowOffset);continue;
    }
