...
export async function nativeAccountProbe(page,binding){
 const start=Date.now(),deadline=Math.min(binding.deadline_epoch_ms,start+30000),observations=[];let opened=false,closed=false,account=null;
 ...
 const inspect=async(readAccount=false)=>{
  remaining();const o=await page.evaluate(({readAccount,operation})=>{
 ...
 try{
  ...
  remaining();await page.locator('[data-tid="MF;cntMain;tlbMainToolbar;btnAvatar"]:visible').click({timeout:remaining()});remaining();opened=true;
  let current;
  for(;;){current=await inspect(true);if(current.menu_count===1)break;if(current.menu_count!==0)throw Error('ACCOUNT_MENU_OWNER_AMBIGUOUS');await page.waitForTimeout(Math.min(100,remaining()));remaining();}
  account=current.account;
  if(!current.line_visible||typeof account!=='string')throw Error('ACCOUNT_LINE_UNOBSERVED');
  await inspect();remaining();await page.keyboard.press('Escape',{timeout:remaining()});remaining();
  for(;;){const after=await inspect();if(after.menu_count===0){closed=true;break;}if(after.menu_count!==1)throw Error('ACCOUNT_MENU_OWNER_AMBIGUOUS');await page.waitForTimeout(Math.min(100,remaining()));remaining();}
  if(account!=='test-2')throw Error('ACCOUNT_MISMATCH');
  return {status:'SUCCEEDED',account,opened,closed,elapsed_ms:Date.now()-start,observations};
 }catch(e){return {status:'FAILED',reason:String(e.message),account,opened,closed,elapsed_ms:Date.now()-start,observations};}
}
...
export function installAccountProbe({Client,Server,CallToolRequestSchema,runDirectory,stateDirectory,overallDeadline}){
 ...
 Server.prototype.setRequestHandler=function(schema,handler){if(schema!==CallToolRequestSchema)return set.call(this,schema,handler);return set.call(this,schema,async(request,extra)=>{
  ...
  const start=performance.now(),deadline=Math.min(start+30000,overallDeadline);need(deadline>start,'Account budget unavailable');
  ...
  const raw=await browser.callTool({name:'browser_run_code_unsafe',arguments:{code}},undefined,{signal,timeout:Math.max(1,Math.floor(deadline-performance.now()))});guard();
