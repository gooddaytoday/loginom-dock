#!/usr/bin/env node
// Independent saved-graph execution. No node wizard or node.apply is used.
...
try {
 ...
 session=await createSession({stateDir:dir,agent:'codex',adapterRevision:'rc-independent-reopen-v1',mode:'executor-replay'});
 ...
 const prepared=await execute(makeWorkspacePrepareCode({loginomUrl:origin+'/app/?testable=true',compatibility:pinned.compatibility,sessionId:session.metadata.sessionId,operationId:'rc-independent-open',intent:'open_package',packagePath:plan.package_path,allowTestLogin:true,testLoginUser:'test-2'}));
 ...
 const index={scope:'rc-independent-saved-graph',model_started:false,settings_reapplied:false,package_saved:false,case:plan.case,run_id:plan.run_id,session:session.metadata,prepare:prepared,geometry,graph_before:before,results:{}};
 for(const label of plan.leaves){
  ...
  try {
   ...
   data=decodeTableOutput(raw,{formatProof,readSettings,expectedColumns:expected[label].columns,requireExactNumbers:true});
 ...
 // Only this isolated reader created these temporary table views. Never write
 // them back into the saved scenario being audited.
 const cleanupOptions={sessionId:session.metadata.sessionId,documentId:prepared.document_id,account:'test-2',packagePath:plan.package_path,
   loginomUrl:origin+'/app/?testable=true',loginomBuild:build,tabTid:prepared.workflow_ref.tab_tid,diagnosticDiscard:true};
 ...
 need(cleanup.status==='SUCCEEDED','Independent reader cleanup remains unconfirmed');
 index.cleanup_response_sequence=sequence;index.package_cleanup=cleanup;await save('index.json',index);
