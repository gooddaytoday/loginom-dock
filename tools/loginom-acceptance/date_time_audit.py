...
def audit(events, request):
    ...
    if configuration['passed'] and request['finish'] == 'execute':
        ...
        checks['raw_output'] = verify_calculator_output(events, request, columns, rows)
        try:
            checkpoints = [e['result'] for e in events if e.get('operation_id') == request['operation_id'] and e.get('phase') == 'node_checkpoint']
            checks['values'] = verify_output(checkpoints[0]['output']['ports'][0], projection)
...
if __name__ == '__main__':
    ...
    result = audit(events, requests[0])
