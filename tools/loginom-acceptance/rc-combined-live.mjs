...
const config=await loadConfig({configPath,stateDir,agent:'codex',adapterRevision:'rc-combined-source-live',mode:'classic'});
if(!stateDir.startsWith(process.cwd()+'/.dock/')||user!=='test-2')throw Error('This RC harness requires owned state and test-2');
const session=await createSession(config),dir=session.directory,bridge=await createBridge(config,session);
const client=new Client({name:'rc-combined-source-operator',version:'1'}),[a,b]=InMemoryTransport.createLinkedPair();
await bridge.server.connect(b);await client.connect(a);let sequence=0,wire,operatorClosed=false;
...
const execute=async code=>{const r=await call('browser_run_code_unsafe',{code});await fs.writeFile(dir+'/browser-'+(++sequence)+'.json',JSON.stringify(r));
 ...
 return value;};
const record=createExecutionJournal({directory:dir,metadata:session.metadata,knownSecrets:[config.apiKey]});
try{
