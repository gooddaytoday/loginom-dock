...
const dir=process.cwd()+'/.dock/union-20260912/live-'+Date.now();await fs.mkdir(dir+'/runtime',{recursive:true});
await fs.symlink(process.env.HOME+'/.loginom-dock/runtime/browsers',dir+'/runtime/browsers');
const session=await createSession({stateDir:dir,agent:'codex',adapterRevision:'Union-20260912-diagnostic',mode:'executor-replay'});
...
const client=new Client({name:'union-live',version:'1'}),transport=new StdioClientTransport({command:process.execPath,
