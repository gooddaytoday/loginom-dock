...
if(!['full','user-v1-component','account-diagnosis'].includes(purpose))throw Error('Unknown acceptance purpose');
const admission=JSON.parse(execFileSync('/usr/bin/python3',[fileURLToPath(new URL('./text_export_readiness.py',import.meta.url)),purpose==='account-diagnosis'?'user-v1-component':purpose],{encoding:'utf8',timeout:30000}));
...
installObserverSdk({Client,Server,CallToolRequestSchema,runDirectory,stateDirectory,run,overallDeadline});
