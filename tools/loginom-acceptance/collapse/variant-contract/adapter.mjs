// Pure diagnostic proposal. No RPC, browser, filesystem, public wiring or authority grant.
import {decodeVariantFrame} from '../variant-prototype/decode.mjs';
...
export function adaptRead(raw,{expected,lifecycle,dateProfile,consistency}={}) {
 ...
 return {contract:'variant-exact-proposal-1',diagnostic_only:true,schema,row_count:raw.row_count,cells,
...
// Proposed user-v1 projection, deliberately NOT the installed compactNodeResult.
export function proposedUserPort(table,{sampleRows=10}={}) {
 ...
 need(new TextEncoder().encode(JSON.stringify(result)).length<=1048576,'diagnostic serialized output limit');
 return result;
}
