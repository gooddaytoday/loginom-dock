...
export function bindObserver({journal,request,run,session,overallDeadline}){
 ...
 need(same(args.target.ref,node)&&same(args.workflow_ref,original.e.parameters.workflow_ref)&&args.document_id===node.document_id&&same(reject.e.parameters.target.ref,node),'Node/workflow owner changed');
 ...
 return {run_id:run.run_id,session_id:session.sessionId,runtime:session.clientRevision,origin:'http://logi-test-plan.bg.local',overall_deadline_ms:overallDeadline,identity,source_edge,workflow_ref:args.workflow_ref,graph_request:{document_id:args.document_id,workflow_ref:args.workflow_ref,target:args.target,inputs:[]},other_pending_operations:[],
