...
await bridge.server.connect(b);await client.connect(a);let sequence=0,wire;
...
const execute=async code=>{const r=await call('browser_run_code_unsafe',{code});await fs.writeFile(dir+'/browser-'+(++sequence)+'.json',JSON.stringify(r));
 ...
 if(r.isError||!raw)throw Error(t.slice(0,1200));return JSON.parse(raw);};
...
try{
 ...
 const ctx={execute,session,dir,fs,record,runtime:wire.runtime,rawRuntime,pinned,call,
 ...
 for await(const line of readline.createInterface({input:process.stdin})){
  if(!line.trim())continue;const c=JSON.parse(line);if(c.operator==='close')break;
