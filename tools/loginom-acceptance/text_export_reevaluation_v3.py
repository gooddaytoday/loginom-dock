...
def reevaluate(run_dir, revision, external_dir=None):
    ...
    return {'version': 'node17-import-origin-contract-v3', 'evaluation_kind': 'versioned_reevaluation_not_original_frozen_audit',
            ...
            'provenance': {'original_files_sha256': {k:digest(v) for k,v in original_files.items()},
                           ...
                           'v2_sources_sha256': {name:digest((WORK/name).read_bytes()) for name in ['text_export_import_contract.py', 'text_export_reevaluation_v3.py', 'text_export_origin.py']},
