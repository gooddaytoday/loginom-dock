...
def audit(r):
    ...
    return dict(passed=True,result_profile='user-v1',scope='real_stdio_user_v1_component_only',projection=projection,native=native,public_projection_negatives=negatives,full_goal_accepted=False,hermes_started=False,evidence_sha256={p:hashlib.sha256((r/p).read_bytes()).hexdigest() for p in paths})
