...
export async function offlineProof(root,{changeResponse=(r)=>r,changeJournal=x=>x,illegalCallAt=null,illegalDirect=false}={}){
 ...
 try{
  hook=installObserverSdk({Client,Server,CallToolRequestSchema,runDirectory:root,stateDirectory:state,run:f.run,overallDeadline});
  ...
  if(!error){const actual=JSON.parse((await readFile(join(root,'observer','actual-dispatch.jsonl'),'utf8')).trim());
   ...
   await writeFile(join(root,'synthetic-expected.json'),JSON.stringify({expected,request:actual.request,run:f.run}));
  }
