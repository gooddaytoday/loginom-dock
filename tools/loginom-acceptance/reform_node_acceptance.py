"""Independent reform-node-complete scenario auditor, not the entire subplan gate."""
import argparse
import hashlib
import json
...
from reform_upload_probe import FIXTURE, descriptor, prompt as render_prompt
...
from calculator_output_evidence import verify_calculator_output as verify_reform_output
from reform_goal_contract import verify_goal,goal_output
...
def audit(request, evidence, prompt, source_bytes):
    ...
    try:
        ...
        goal = WORK/'goals/reform-node-complete.txt'
        check('goal_identity', request.get('goal_id') == 'reform-node-complete'
        ...
        check('five_node_operations',len(declarations)==5)
        if len(declarations)!=5:return report(checks)
        seed,initial_group,calc,reopened,calc_reopened=[e['request'] for e in declarations]
        checks['declared_task']=verify_goal(seed,initial_group,calc,source_bytes)
        for label,r in [('seed',seed),('reopened',reopened)]:
            checks[label+'_configuration']=verify_configuration_readback(evidence['events'],r)
        for label,r,final in [('initial_reform',initial_group,False),('changed_reform',calc,True),('reform_reopened',calc_reopened,True)]:
            checks[label+'_configuration']=verify_reform_configuration(evidence['events'],r)
            checks[label+'_output']=verify_reform_output(evidence['events'],r,*goal_output(source_bytes,final))
        for label,r in [('seed',seed),('initial_reform',initial_group),('changed_reform',calc),('reopened',reopened),('reform_reopened',calc_reopened)]:
        ...
        checks['public_nodes_saves'] = verify_public_nodes_and_saves(evidence,
            {r['operation_id']:r for r in (seed,initial_group,calc,reopened,calc_reopened)}, [e['operation_id'] for e in saves],allow_validation_refusals=True)
        ...
        def node_result(r):
            ...
            return found[0]
        initial_import,first_group,initial_calc,opened_import,opened_calc=map(node_result,(seed,initial_group,calc,reopened,calc_reopened))
        check('one_input_link',initial_group['inputs']==[dict(source=initial_import['node'],output=0,input=0)]
            and calc['inputs'] in ([],initial_group['inputs']))
        check('same_reform_node',first_group['node']==initial_calc['node'] and calc['target']['ref']==first_group['node'])
        ...
        source_port=dict(node_label='Данные',tids=['Данные;Input_Connection[0]','Данные;Input_Var[0]','Данные;Output_Data[0]'])
        calc_port=dict(node_label='Очистка',tids=['Очистка;Input_Data[0]','Очистка;Output_Data[0]'])
        graphs=[dict(nodes=['Данные'],ports=[source_port],links=[]),
            dict(nodes=['Данные','Очистка'],ports=[source_port,calc_port],links=['Данные|Output_Data[0]|Очистка|Input_Data[0]'])]
        ...
        if checks['saves']['passed']:
            ...
            checks['reform_reopen_binding']=verify_reopen_binding(evidence,calc,calc_reopened,save_id,path,reform=True)
        ...
        def semantics(result):
            ...
            def normalized(value):
                ...
                if isinstance(value,dict):return {k:normalized(v) for k,v in value.items() if k not in ('field_id','source_field_id')}
                return value
            return normalized({k:v for k,v in rb.items() if k not in ('node','receipt_ids')})
        check('persisted_reform_settings',semantics(initial_calc)==semantics(opened_calc))
        check('fresh_reform_execution',initial_calc['execution']['execution_id']!=opened_calc['execution']['execution_id'])
        check('reopened_reform_unchanged',calc_reopened['parameters']=={'changes':[]}
        ...
        allowed_ids = {seed['operation_id'], initial_group['operation_id'], calc['operation_id'], reopened['operation_id'], calc_reopened['operation_id'], *[e['operation_id'] for e in saves]} | read_ids
        ...
        check('all_components_ran', all(k in checks for k in ('delivery_bytes_output','persisted_import','reform_reopen_binding')))
...
def report(checks):
    ...
    return dict(passed=passed, checks=checks, scope='reform-node-complete-scenario', subplan_complete=False)
...
def audit_directory(directory):
    ...
    required = {p.relative_to(WORK).as_posix() for pattern in ('*.py', '*.mjs') for p in WORK.glob(pattern)} | {FIXTURE, 'goals/reform-node-complete.txt'}
...
if __name__ == '__main__':
    ...
    (args.run_dir/'reform-node-audit.json').write_text(json.dumps(result, ensure_ascii=False, indent=2)+'\n')
