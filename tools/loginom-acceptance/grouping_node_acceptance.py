"""Independent grouping-node-complete scenario auditor, not the entire subplan gate."""
import argparse
import hashlib
import json
...
from grouping_upload_probe import FIXTURE, descriptor, prompt as render_prompt
...
from grouping_goal_contract import verify_goal,goal_output
...
def audit(request, evidence, prompt, source_bytes):
    ...
    try:
        ...
        goal = WORK/'goals/grouping-node-complete.txt'
        check('goal_identity', request.get('goal_id') == 'grouping-node-complete'
        ...
        for label,r,final in [('initial_grouping',initial_group,False),('changed_grouping',calc,True),('grouping_reopened',calc_reopened,True)]:
            checks[label+'_configuration']=verify_grouping_configuration(evidence['events'],r)
            checks[label+'_output']=verify_grouping_output(evidence['events'],r,*goal_output(source_bytes,final))
        for label,r in [('seed',seed),('initial_grouping',initial_group),('changed_grouping',calc),('reopened',reopened),('grouping_reopened',calc_reopened)]:
        ...
        check('one_input_link',all(r['inputs']==[dict(source=initial_import['node'],output=0,input=0)] for r in (initial_group,calc)))
        check('same_grouping_node',first_group['node']==initial_calc['node'] and calc['target']['ref']==first_group['node'])
        ...
        calc_port=dict(node_label='Итоги',tids=['Итоги;Input_Data[0]','Итоги;Output_Data[0]'])
        graphs=[dict(nodes=['Данные'],ports=[source_port],links=[]),
            dict(nodes=['Данные','Итоги'],ports=[source_port,calc_port],links=['Данные|Output_Data[0]|Итоги|Input_Data[0]'])]
        ...
        if checks['saves']['passed']:
            ...
            checks['grouping_reopen_binding']=verify_reopen_binding(evidence,calc,calc_reopened,save_id,path,grouping=True)
        ...
        check('persisted_grouping_settings',semantics(initial_calc)==semantics(opened_calc))
        check('fresh_grouping_execution',initial_calc['execution']['execution_id']!=opened_calc['execution']['execution_id'])
        check('reopened_grouping_unchanged',calc_reopened['parameters']=={}
        ...
        check('all_components_ran', all(k in checks for k in ('delivery_bytes_output','persisted_import','grouping_reopen_binding')))
...
def report(checks):
    ...
    return dict(passed=passed, checks=checks, scope='grouping-node-complete-scenario', subplan_complete=False)
...
def audit_directory(directory):
    ...
    required = {p.relative_to(WORK).as_posix() for pattern in ('*.py', '*.mjs') for p in WORK.glob(pattern)} | {FIXTURE, 'goals/grouping-node-complete.txt'}
...
if __name__ == '__main__':
    ...
    (args.run_dir/'grouping-node-audit.json').write_text(json.dumps(result, ensure_ascii=False, indent=2)+'\n')
