...
def diagnostics(before_events, final, index, base, package_path, revision):
    ...
    try:
        ...
        nodes = {n['label']: n for n in state['nodes']}
        expected_links = [dict(source=nodes[parent]['ref']['node_id'], target=nodes[child]['ref']['node_id'], input=0, output=0)
                          for child, parent in PARENTS.items()]
        checks['raw_saved_graph'] = dict(passed=set(nodes) == set(LABELS) and len(state['nodes']) == 6
        ...
        positions = {e['operation_id']: i for i, e in enumerate(after) if e.get('phase') == 'node_apply_prepared'}
        checks['upstream_before_downstream'] = dict(passed=all(positions[index['operations'][parent]] < positions[index['operations'][child]] for child, parent in PARENTS.items()))
        for label in LABELS:
        ...
        checks['no_diagnostic_reconfiguration'] = dict(passed=len([e for e in after if e.get('phase') == 'node_apply_prepared']) == 6
            and not any(e.get('phase') == 'prepared' and e.get('action_key', '').startswith('package.save') for e in after))
