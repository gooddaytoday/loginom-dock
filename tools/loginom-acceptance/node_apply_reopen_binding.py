...
def verify_reopen_binding(evidence, seed_request, request, save_operation_id, path):
    failures = []
    ...
    if (request.get('target', {}).get('kind') != 'existing'
            ...
            or not unchanged_patch(seed_request.get('parameters', {}).get('settings', {}), request.get('parameters', {}).get('settings'))):
