...
def verify_reopen_binding(evidence, seed_request, request, save_operation_id, path, *, calculator=False, grouping=False, sorting=False, reform=False, row_filter=False):
    failures = []
    ...
    if (request.get('target', {}).get('kind') != 'existing'
            ...
            or not (request.get('parameters')=={'changes':[]} if reform else request.get('parameters')=={} if grouping or sorting or row_filter else request.get('parameters')=={'expressions':[]} if calculator else unchanged_patch(seed_request.get('parameters', {}).get('settings', {}), request.get('parameters', {}).get('settings')))):
