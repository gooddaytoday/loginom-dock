"""Independent node-apply-complete scenario auditor, not the entire subplan gate."""
import argparse
import hashlib
import json
...
def audit(request, evidence, prompt, source_bytes):
    ...
    try:
        ...
        goal = WORK/'goals/node-apply-complete.txt'
        check('goal_identity', request.get('goal_id') == 'node-apply-complete'
        ...
        check('two_import_operations', len(declarations) == 2)
        if len(declarations) != 2:
            return report(checks)
        seed, reopened = [e['request'] for e in declarations]
        checks['seed_configuration_readback'] = verify_configuration_readback(evidence['events'], seed)
        checks['reopened_configuration_readback'] = verify_configuration_readback(evidence['events'], reopened)
        checks['seed_workflow_activation'] = verify_workflow_activation(evidence['events'], seed)
        checks['reopened_workflow_activation'] = verify_workflow_activation(evidence['events'], reopened)
        ...
        checks['public_nodes_saves'] = verify_public_nodes_and_saves(evidence,
            {seed['operation_id']: seed, reopened['operation_id']: reopened}, [e['operation_id'] for e in saves])
        ...
        checks['persistence'] = verify_sales_persistence(evidence, seed, reopened, run_id, directory, source_bytes, path, SAVE_REVISIONS)
        ...
        allowed_ids = {seed['operation_id'], reopened['operation_id'], *[e['operation_id'] for e in saves]} | read_ids
        ...
        check('all_components_ran', 'delivery_bytes_output' in checks)
...
def report(checks):
    ...
    return dict(passed=passed, checks=checks, scope='node-apply-complete-scenario', subplan_complete=False)
...
def audit_directory(directory):
    ...
    required = {p.relative_to(WORK).as_posix() for pattern in ('*.py', '*.mjs') for p in WORK.glob(pattern)} | {FIXTURE, 'goals/node-apply-complete.txt'}
...
if __name__ == '__main__':
    ...
    (args.run_dir/'node-apply-audit.json').write_text(json.dumps(result, ensure_ascii=False, indent=2)+'\n')
