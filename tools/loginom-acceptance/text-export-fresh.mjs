...
if(process.argv.includes('--copy-to'))throw Error('Fresh original audit cannot copy a package');
const modelDir=option('--model-run');
...
const execute=async code=>{
 const id=++sequence,started_at=new Date().toISOString(),requestBody=JSON.stringify({code});
 await fs.writeFile(dir+'/browser-request-'+id+'.json',requestBody);
 const r=await client.callTool({name:'browser_run_code_unsafe',arguments:{code}},undefined,{timeout:60000});
 const responseBody=JSON.stringify(r);await fs.writeFile(dir+'/browser-'+id+'.json',responseBody);
 await fs.appendFile(dir+'/browser-ledger.jsonl',JSON.stringify({sequence:id,started_at,completed_at:new Date().toISOString(),request_sha256:digest(requestBody),response_sha256:digest(responseBody)})+'\n');
...
try {
 ...
 const originalFiles={};for(const name of ['request.json','scenario.txt','evidence.json','full-audit.json'])originalFiles[name]=digest(await fs.readFile(modelDir+'/'+name));
 await fs.writeFile(dir+'/fresh-binding.json',JSON.stringify({version:1,model_run_directory:modelDir,original_files_sha256:originalFiles,preparation_sequence:sequence,package_path:packagePath,account:user,storage_directory:storage,loginom_url:loginomUrl,session_id:session.metadata.sessionId,harness_sha256:digest(await fs.readFile(new URL(import.meta.url)))},null,2));
 ...
 if(process.argv.includes('--copy-to')||prep.workflow_ref.navigation_path.some(c=>c.label.endsWith('(только чтение)'))){
  if(!process.argv.includes('--copy-to')){
   const cleanup=await execute(`async page=>{const tab=${JSON.stringify(prep.workflow_ref.tab_tid)},name=${JSON.stringify(prep.workflow_ref.navigation_path[2].label)};await page.locator('[data-tid="MF;cntMain;tlbMainToolbar;btnPackagesMenu"]').click();const header=page.locator('[data-tid="MF;MainMenuForm;pnlSaveClosePackage;p.h;p.t"]');await header.waitFor();const actual=(await header.innerText()).trim();if(actual.replace(' (только чтение)','')!==name.replace(' (только чтение)',''))throw Error('Package ownership mismatch: '+actual);await page.locator('[data-tid="MF;MainMenuForm;btnClosePackage"]').click();await page.locator('[data-tid="'+tab+'"]').waitFor({state:'hidden'});await page.locator('[data-tid="MF;cntMain;tlbMainToolbar;btnAvatar"]').click();const account=(await page.locator('[data-tid="MF;AppMenuForm;p.h;p.t"]').innerText()).trim();if(account!==${JSON.stringify(user)})throw Error('Wrong account');await page.locator('[data-tid="MF;AppMenuForm;btnLogOut"]').click();await page.locator('[data-tid="LoginForm;Login;edtUsername"] input').waitFor({state:'visible'});return {package_closed:true,logged_out:true,account};}`);
   await fs.writeFile(dir+'/readonly-session-cleanup.json',JSON.stringify(cleanup,null,2));
   console.log(JSON.stringify({dir,status:'READ_ONLY',cleanup}));throw Error('Original remains read-only; own package closed and session logged out');
  }
