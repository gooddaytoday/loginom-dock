...
export function syntheticBinding(){
 ...
 const meta={session_id:'session',runtime_revision:'a'.repeat(64)};
...
export function syntheticNativeResponses(c){
 ...
 return [graph,obs([toolbar]),obs([files]),gesture,obs([panel]),obs([panel]),obs([file]),obs([file]),download,obs([workspace]),obs([tab]),gesture,graph].map(x=>structuredClone(x));
}
