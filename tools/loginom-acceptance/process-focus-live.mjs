...
const dir=process.cwd()+'/.dock/process-node-focus-20260911/live-'+Date.now();await fs.mkdir(dir+'/runtime',{recursive:true});
await fs.symlink(process.env.HOME+'/.loginom-dock/runtime/browsers',dir+'/runtime/browsers');
const session=await createSession({stateDir:dir,agent:'codex',adapterRevision:'ProcessNodeFocus-20260911-diagnostic',mode:'executor-replay'});
...
const client=new Client({name:'process-focus-live',version:'1'}),transport=new StdioClientTransport({command:process.execPath,
