...
if(!plan||Object.entries(report.checks).some(([k,v])=>k!=='independent_reopen_present'&&!v.passed)||!plan.package_path.startsWith('/test-4/packages/')||plan.runtime_revision!=='a9db4113ac69d38d7227e971ece3652acf3e1bf836723e6e577917eda916406f')throw Error('Verified working phase required before independent reopen');
...
try{
 ...
 const index={prepare:prepared,geometry,runtime_revision:session.metadata.clientRevision,manifest_sha256:plan.manifest_sha256,results:{}};let i=0;
 ...
 await save('index.json',index);console.log(JSON.stringify({scope:'independent_full_reopen',results:Object.keys(index.results).length,output:dir,settings_reapplied:false,package_saved:false}));
