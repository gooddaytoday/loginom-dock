"""Independent filter-node-complete scenario auditor, not the entire subplan gate."""
import argparse
import hashlib
import json
...
from filter_upload_probe import FIXTURE, descriptor, prompt as render_prompt
...
from filter_goal_contract import verify_goal,goal_output
...
def audit(request, evidence, prompt, source_bytes):
    ...
    try:
        ...
        goal = WORK/'goals/filter-node-complete.txt'
        check('goal_identity', request.get('goal_id') == 'filter-node-complete'
        ...
        for label,r,final in [('initial_filter',initial_group,False),('changed_filter',calc,True),('filter_reopened',calc_reopened,True)]:
            checks[label+'_configuration']=verify_filter_configuration(evidence['events'],r)
            checks[label+'_output']=verify_filter_output(evidence['events'],r,*goal_output(source_bytes,final))
        for label,r in [('seed',seed),('initial_filter',initial_group),('changed_filter',calc),('reopened',reopened),('filter_reopened',calc_reopened)]:
        ...
        check('same_filter_node',first_group['node']==initial_calc['node'] and calc['target']['ref']==first_group['node'])
        ...
        calc_port=dict(node_label='Отбор',tids=['Отбор;Input_Data[0]','Отбор;Output_Data[0]','Отбор;Output_Data[1]'])
        graphs=[dict(nodes=['Данные'],ports=[source_port],links=[]),
            dict(nodes=['Данные','Отбор'],ports=[source_port,calc_port],links=['Данные|Output_Data[0]|Отбор|Input_Data[0]'])]
        ...
        if checks['saves']['passed']:
            ...
            checks['filter_reopen_binding']=verify_reopen_binding(evidence,calc,calc_reopened,save_id,path,row_filter=True)
        ...
        check('persisted_filter_settings',filter_settings(initial_calc)==filter_settings(opened_calc))
        check('fresh_filter_execution',initial_calc['execution']['execution_id']!=opened_calc['execution']['execution_id'])
        check('reopened_filter_unchanged',calc_reopened['parameters']=={}
        ...
        check('all_components_ran', all(k in checks for k in ('delivery_bytes_output','persisted_import','filter_reopen_binding')))
...
def report(checks):
    ...
    return dict(passed=passed, checks=checks, scope='filter-node-complete-scenario', subplan_complete=False)
...
def audit_directory(directory):
    ...
    required = {p.relative_to(WORK).as_posix() for pattern in ('*.py', '*.mjs') for p in WORK.glob(pattern)} | {FIXTURE, 'goals/filter-node-complete.txt'}
...
if __name__ == '__main__':
    ...
    (args.run_dir/'filter-node-audit.json').write_text(json.dumps(result, ensure_ascii=False, indent=2)+'\n')
