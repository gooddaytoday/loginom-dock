...
class DataPipelineTest(unittest.TestCase):
    def test_transfer_prefix_survives_later_pipeline_mutations_but_never_admits_domain(self):
        ...
        evidence['calls'].append({'tool':PREFIX+'dock_ui_action','row':20,'arguments':{}})
        report=data_pipeline.audit(evidence,[],request,PREFIX,MUTATIONS,file_storage_inspect)
    ...
    def test_mutation_before_resolved_inspection_does_not_prove_transfer(self):
        evidence,request=UploadVerifyTest().fixture()
        evidence['calls'].append({'tool':PREFIX+'dock_ui_action','row':14,'arguments':{}})
        report=data_pipeline.audit(evidence,[],request,PREFIX,MUTATIONS,file_storage_inspect)
