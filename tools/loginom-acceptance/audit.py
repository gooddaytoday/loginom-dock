...
def audit(request, evidence, prompt):
    ...
    try:
        ...
        expected_prompt=upload_probe.prompt(goal.read_text(),path,destination,run_id) if goal_id in ('file-upload-probe','file-upload-verify','data-pipeline','import-roundtrip') else render_goal(goal.read_text(),path,destination)
        ...
        check('upload_only_in_declared_probe',goal_id in ('file-upload-probe','file-upload-verify','data-pipeline','import-roundtrip') or not any(c['tool'] in (PREFIX+'dock_artifact_upload',PREFIX+'dock_artifact_verify') for c in calls))
...
def audit_directory(run):
    ...
    if request.get('goal_id') in ('checkbox-roundtrip','context-menu-checkbox','root-checkbox','file-storage-inspect','file-upload-probe','file-upload-verify','data-pipeline','import-roundtrip'):
    ...
    if request.get('goal_id') in ('file-upload-probe','file-upload-verify','data-pipeline','import-roundtrip'):
    ...
    if request.get('goal_id') in ('file-upload-verify','data-pipeline','import-roundtrip'):
    ...
    if request.get('goal_id') in ('data-pipeline','import-roundtrip'):
        frozen=frozen and all(request.get('harness_inputs',{}).get(name)==sha(Path(__file__).parent.joinpath(name).read_bytes()) for name in ('data_pipeline.py','rendered_results.py','settings_evidence.py','import_settings_evidence.py','import_roundtrip_evidence.py',*data_pipeline.FIXTURES))
