...
def audit(request, evidence, prompt):
    ...
    try:
        ...
        if goal_id not in ('basic-graph','auto-link-retain','auto-link-remove','palette-inventory','checkbox-roundtrip'):
        ...
        if goal_id=='checkbox-roundtrip':
            return checked_state.audit_goal(evidence, checks, PREFIX, MUTATIONS)
...
def audit_directory(run):
    ...
    if request.get('goal_id') == 'checkbox-roundtrip':
