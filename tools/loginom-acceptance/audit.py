...
def audit_directory(run):
    ...
    if request.get('goal_id')=='data-pipeline':
        frozen=frozen and all(request.get('harness_inputs',{}).get(name)==sha(Path(__file__).parent.joinpath(name).read_bytes()) for name in ('data_pipeline.py','rendered_results.py','settings_evidence.py',*data_pipeline.FIXTURES))
