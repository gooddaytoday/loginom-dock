...
const assignment='node17:final-admission-preflight:1:79e648b2',need=(x,m)=>{if(!x)throw Error(m)},hash=x=>createHash('sha256').update(x).digest('hex');
...
const args=[join(work,'text-export-observer-client.mjs'),'--config',configPath,'--state-dir',stateDir,'--agent','hermes','--adapter-revision','node17-user-v1-component','--mode','executor-replay','--action-manifest-uri',manifest.catalog.uri,'--action-manifest-sha256',manifest.catalog.sha256,'--replay-bootstrap','--replay-login-user','test-2','--replay-loginom-url',run.loginom_url,'--input-artifact',JSON.stringify(artifact)];
await save('entry-launch.json',{command:process.execPath,args,environment:{DOCK_ACCEPTANCE_RUN_DIR:runDir,DOCK_ACCEPTANCE_PURPOSE:'user-v1-component',DOCK_ACCEPTANCE_DEADLINE_EPOCH_MS:String(epochDeadline)}});
const transport=new StdioClientTransport({command:process.execPath,args,cwd:root,stderr:'pipe',env:{...process.env,DOCK_ACCEPTANCE_RUN_DIR:runDir,DOCK_ACCEPTANCE_PURPOSE:'user-v1-component',DOCK_ACCEPTANCE_DEADLINE_EPOCH_MS:String(epochDeadline)}});
...
try{
 ...
 const one=xs=>{need(xs.length===1,'Unique observed UI control required');return xs[0]};
 const control=async tid=>{const roots=(await wire('dock_workspace_observe',{scope:'roots'})).output;const element=one(roots.ui.elements.filter(e=>e.tid===tid));return (await wire('dock_workspace_observe',{root_ref:element.ref,observation_id:roots.observation_id})).output;};
 const avatar=async id=>{const s=await control('MF;cntMain;tlbMainToolbar');const e=one(s.ui.elements.filter(e=>e.tid==='MF;cntMain;tlbMainToolbar;btnAvatar'));const result=await wire('dock_ui_action',{operation_id:id,observation_id:s.observation_id,action:{verb:'click',ref:e.ref}});need(result.status==='SUCCEEDED','Avatar gesture incomplete');};
 await avatar('component-account-open');let menu=(await wire('dock_workspace_observe',{scope:'all'})).output;const pages=[menu],found=s=>s.ui.elements.some(e=>/test-2/.test(e.label??''));
 while(!found(menu)&&menu.page?.next_cursor){need(pages.length<32,'Account observation page budget');const previous=menu;menu=(await wire('dock_workspace_observe',{cursor:menu.page.next_cursor})).output;need(menu.observation_id===previous.observation_id&&menu.dom_epoch.document===previous.dom_epoch.document,'Account observation changed');pages.push(menu);}
 await save('account-observations.json',pages);await save('account-observation.json',menu);need(found(menu),'Observed test-2 account required');await avatar('component-account-close');
