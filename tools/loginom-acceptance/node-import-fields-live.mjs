// Operator-only source-runtime QA for the configuration phase with addressed
// import definitions. Does not attest delivery, output or package persistence.
...
import {configureTextImportFields,validateTextImportFieldsRequest} from '../../client/lib/text-import-procedure.mjs';
...
validateTextImportFieldsRequest(settings);
const url=new URL(loginomUrl);if(url.username||url.password)throw Error('Do not put credentials in URL');
const dir=process.cwd()+'/.dock/text-import-v3/fields-'+Date.now();await fs.mkdir(dir+'/runtime',{recursive:true});await fs.symlink(process.env.HOME+'/.loginom-dock/runtime/browsers',dir+'/runtime/browsers');
const session=await createSession({stateDir:dir,agent:'codex',adapterRevision:'text-import-fields-qa-v1',mode:'executor-replay'});
...
const client=new Client({name:'node-procedure-fields-qa',version:'1'});
...
try {
 ...
 const operation={id:'settings-fields',action:{action_key:'node.configure_text_import.fields.internal',revision:'1'},deadline:Date.now()+240000,
   checkpoint:{document_id:initial.output.dom_epoch.document,workflow_ref:initial.output.workflow_ref}};
 const channel=createNodeProcedure({operation,targetOrigin:url.origin,targetBuild:'7.4.2',maxSteps:2048,
 record,wrapMutation:(code,ref)=>withBrowserReceipt('('+code+')(page)',{receipt_namespace:session.metadata.sessionId,receipt_id:ref.id,receipt_signature:ref.signature,operation_id:ref.id}),execute});
 const configured=await configureTextImportFields(channel,settings,initial.output.wizard.owner_context);
 await fs.writeFile(dir+'/configured.json',JSON.stringify(configured,null,2));
 const summary={status:'PASS',scope:'source_format_and_paged_import_definitions',operation_id:operation.id,steps:channel.steps,
   upload_verified:false,execution_verified:false,package_saved:false,hermes_acceptance:false};
