...
function fixture(){
 ...
 const history=[{cleanup_confirmed:true,request:{operation_id:'import',target:{kind:'existing',type:'imports.text',ref:node},parameters:{source,settings:{source:{source_path:proof.destination}}}},
...
test('completed private import resolves artifact provenance and cannot trust a path alone',()=>{
 const f=fixture(),value=completedStaticImports(f.history,f.uploads,ctx)[0];
...
])test('static provenance rejects '+name,()=>{const f=fixture();change(f);assert.throws(()=>completedStaticImports(f.history,f.uploads,ctx));});
