...
function fixture({loseReply=false}={}){
 ...
 const channel={observe:async({ready})=>{observations++;const s=structuredClone(snapshot());assert.ok(ready(s));return s;},perform:async({initialObservation,resolve})=>{
  ...
  const [id,flag]=action.ref.split('-'),row=matrices[selected].find(r=>r.record_id===id);row[flags[flag]]=!row[flags[flag]];fields.find(f=>f.name===selected).count+=row[flags[flag]]?1:-1;
