...
import {adaptCell,adaptRead,nativeUserPort,temporalProfile} from '../lib/variant-native-values.mjs';
...
function fixture(n=15){
 ...
 const raw={empty_count_attested:n===0,read_id:'r1',method:321,interface:116,port:0,document_id:'d',workflow_id:'w',package_id:'pkg',node_id:'n',port_guid:'p',source:{owner:0,object:9},execution:{status:'completed',execution_id:'d:1:2'},schema:[{name:'Values',label:'Values',type:6}],row_count:n,owner_rechecked:true,cache_identity_rechecked:true,
...
test('table completeness and public 10-row sample are separate after JSON roundtrip',()=>{
 const f=fixture();const t=adaptRead(f.raw,f.context),u=JSON.parse(JSON.stringify(nativeUserPort(t)));
...
test('only scalar column of a four-column table is partial; DataTypes absence is irrelevant',()=>{
 ...
 const t=adaptRead(f.raw,f.context);assert.equal(t.coverage.table_complete,false);assert.equal(nativeUserPort(t).sample_rows,0);
});
...
test('full projection refuses oversized serialization rather than silently truncating',()=>{
 const f=fixture();const t=adaptRead(f.raw,f.context);t.cells=t.cells.map(c=>({...c,value:'x'.repeat(100000)}));assert.throws(()=>nativeUserPort(t),/serialized output limit/);
});
