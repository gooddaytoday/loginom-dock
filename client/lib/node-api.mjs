...
export const nodeApplyInputSchema=object({
 ...
 target:object({kind:choice('new','existing'),type:choice('imports.text','transform.calculator','transform.group_data','transform.sorting','transform.reform_columns','transform.filter_data'),label:text(200),ref,
 ...
 mode:choice('delimited','expression','aggregate','keys','scalar','conditions'),parameters:object({
  ...
  expressions:array(calculatorExpressionSchema,128),order:array(text(128),128,1),group_by:array(groupingFieldSchema,128,1),measures:array(groupingMeasureSchema,256,1),keys:array(sortingKeySchema,128,1),compare_with_locale:boolean,changes:array(reformChangeSchema,128),groups:filterParametersSchema.properties.groups},[]),
 mappings:array(object({direction:choice('input','output'),port:integer(0,1),autosync:boolean,
  fields:array(object({source:object({kind:choice('configured_field'),name:text(128)}),name:text(128),label:text(120),excluded:boolean},['source']),1000,1)},['direction','port']),3),
