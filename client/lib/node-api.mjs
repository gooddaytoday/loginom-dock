...
export const nodeApplyInputSchema=object({
 ...
 target:object({kind:choice('new','existing'),type:choice('imports.text','transform.calculator','transform.group_data','transform.sorting','transform.reform_columns','transform.filter_data','transform.join_data'),label:text(200),ref,
  position:object({x:{type:'number',minimum:8,maximum:10000},y:{type:'number',minimum:8,maximum:10000}})},['kind','type']),
 inputs:array(object({source:ref,output:integer(0,99),input:integer(0,99)}),2),
 mode:choice('delimited','expression','aggregate','keys','scalar','conditions','inner','left'),parameters:object({
