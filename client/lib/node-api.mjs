...
export const nodeApplyInputSchema=object({
 ...
 target:object({kind:choice('new','existing'),type:choice('imports.text','transform.calculator','transform.group_data','transform.sorting'),label:text(200),ref,
 ...
 mode:choice('delimited','expression','aggregate','keys'),parameters:object({
  ...
  expressions:array(calculatorExpressionSchema,128),order:array(text(128),128,1),group_by:array(groupingFieldSchema,128,1),measures:array(groupingMeasureSchema,256,1),keys:array(sortingKeySchema,128,1),compare_with_locale:boolean},[]),
