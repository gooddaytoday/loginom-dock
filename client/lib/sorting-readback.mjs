const need=(v,m)=>{if(!v)throw Error('Sorting readback: '+m);};
const pick=(v,keys)=>Object.fromEntries(keys.map(k=>[k,v[k]]));
export function sortingConfigurationReadback({node,phases,operation_id}){
 ...
 need(c?.verified&&c.inventory_complete&&owner(c.node_context)&&c.keys?.length,'sorting configuration owner');
 need(configured.value.validation?.status==='accepted_by_loginom_next'&&owner(configured.value.validation.node_context),'sorting validation');
 need(saved.value.settings_applied===true&&saved.value.mode==='done'&&owner(saved.value.node_context)
  &&finished.value.settings_applied===true&&['done','execute'].includes(finished.value.mode)&&owner(finished.value.node_context),'sorting finish');
 ...
 return {kind:'sorting',scope:'observed_before_verified_finish',values_are:'observed_ui_values',node:structuredClone(node),receipt_ids:[input,configured,saved,mapped,finished].map(p=>p.receipt_id),mode:'keys',
  keys:c.keys.map(f=>pick(f,['name','label','type','order','direction','case_sensitive'])),comparison:structuredClone(c.comparison),
