...
export function createDateTimeNodeSupport(config){return createTabularTransformNodeSupport(config,{
 ...
 async configure(channel,p,{request,inputMapping,preconfiguration}){
  ...
  let removed=[];
  const changed=await configureDateTime(channel,p,{inputMapping,...(preconfiguration?{beforeChanges:original=>{
