...
export function createDateTimeNodeSupport(config){return createTabularTransformNodeSupport(config,{
 ...
 async configure(channel,p,{request,inputMapping}){
  ...
  const changed=await configureDateTime(channel,p,{inputMapping});if(request.finish==='close')return changed;
  ...
  if(destination.wizard.stage==='output_mapping'){
   inline_mapping=await configureDateTimeOutput(channel,changed.configuration,p,{});await advance('output_mapping','done');
  }
