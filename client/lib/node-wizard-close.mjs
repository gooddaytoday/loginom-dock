...
export function wizardCloseBinding(state) {
 ...
 const output=n?.output_port?.direction==='output'&&Number.isInteger(n.output_port.port)&&n.output_port.port>=0
  &&n.output_port.port<100&&Number.isInteger(n.output_port.native_index)&&n.output_port.native_index>=0
  &&typeof n.output_port.port_guid==='string'&&n.output_port.port_guid.length>0&&typeof n.output_port.opening_operation_id==='string'&&n.output_port.opening_operation_id.length>0&&w?.stage==='output_mapping';
 ...
 return {kind:'close',root_ref:w.root_ref,root_tid:w.root_tid,stage:w.stage,
  owner:structuredClone(input?{input_port:n.input_port}:output?{output_port:n.output_port}:w.owner_context),node:{document_id:n.document_id,workflow_id:n.workflow_id,node_id:n.node_id}};
}
...
export function wizardCloseDialogOwner(state,binding) {
 ...
 if(w?.status!=='observed'||w.root_ref!==binding.root_ref||w.root_tid!==binding.root_tid
  ||w.stage!==binding.stage||!same(binding.owner?.input_port?{input_port:n?.input_port}:binding.owner?.output_port?{output_port:n?.output_port}:w.owner_context,binding.owner)||n?.verified!==true
