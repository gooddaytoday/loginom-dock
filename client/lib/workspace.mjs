...
export const workspaceObserveTool = {
  ...
  description: 'Read a compact page of the prepared Loginom workspace. Before preparation, scope bootstrap reads only application/build/login/blocker state without navigation, login, draft creation or capture activation. Otherwise choose all (default), palette, graph or dialogs. Optionally supply root_ref plus its observation_id to limit detailed UI elements/cells to an already delivered element and its descendants. Detailed traversal is limited to the root; global context and blockers use fixed native queries. Graph completeness is not established by a root read. Use scope roots to discover regions in an oversized document without detailed traversal. Region refs have no gestures: first read one using root_ref and observation_id, then act on its delivered controls. Discovery is not proof that blockers are absent. Continue with cursor=page.next_cursor alone; pages share observation_id and revision. If the workspace changes, start a new observation. Only delivered UI refs can be used. visible means rendered, not necessarily reachable: prefer interaction.state=point_observed for gestures. outside_viewport and point_not_observed indicate that no interaction point was seen; read other pages to find a reachable target, especially after scroll. Truncation and full_dom_complete=false mean this is not proof of the whole workspace. Read-only.',
