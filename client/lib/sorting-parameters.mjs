...
export function validateSortingParameters(p,mode,r){
 ...
 need(p&&typeof p==='object'&&!Array.isArray(p),'Invalid parameters.parameters: sorting settings must be an object');
 const extra=Object.keys(p).find(k=>!['keys','compare_with_locale'].includes(k));
 need(extra===undefined,'Invalid parameters.parameters.'+extra+': unknown sorting setting; case_sensitive belongs to each string/variant key');
...
export function resolveSortingParameters(p,fields){
 ...
 return p.keys.map((k,index)=>{const matches=fields.filter(f=>f.name===k.field.name);need(matches.length===1,'Sorting input field is missing: '+k.field.name);
  const f=matches[0],text=['string','variant'].includes(f.type);
  need(text?typeof k.case_sensitive==='boolean':k.case_sensitive===undefined,'Invalid parameters.parameters.keys['+index+'].case_sensitive: '+(text?'explicit boolean required for '+f.type+' field '+f.name:'omit for '+f.type+' field '+f.name));
