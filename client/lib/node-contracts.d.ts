...
export type NodeType = 'preprocessing.data_recovery' | 'imports.text' | 'transform.calculator' | 'transform.reform_columns'
...
export interface NodeHandler<T extends NodeType, P> {
  ...
  configurationReadback?(context: {node: NodeRef; operation_id: string;
    phases: Array<PhaseReceipt & {value: VerifiedNodePhase}>}): TextImportConfigurationReadback | CalculatorConfigurationReadback | GroupingConfigurationReadback | SortingConfigurationReadback | MissingValuesConfigurationReadback | JoinConfigurationReadback;
}
