...
const uiActionTool = { name: 'dock_ui_action',
  description: 'Perform one bounded UI gesture on a fresh ref from dock_workspace_observe: click, double_click, fill, press, drag, scroll or set_checked. action uses verb, e.g. {verb:"click",ref:"<observed ui-ref>"}, {verb:"drag",source_ref:"<observed ui-ref>",target_ref:"<observed ui-ref>"} or {verb:"scroll",ref:"<observed ui-ref>",delta_y:300}. Scroll requires allowed_actions including scroll and targets only its observed scroll owner; delta_y is a nonzero integer within -1000..1000. For a checkbox/radio use {verb:"set_checked",ref:"<observed ui-ref>",checked:true}; check_state is read before/after and an already satisfied request does not click. Radio options can only be selected, not independently unchecked. Read the new observation after scrolling; old refs/pages are invalid. Use for settings, execution, inspection and repairs outside the ready-made actions. No JavaScript or selectors. A successful gesture is not proof of task completion: inspect its result. operation_id is a NEW unique UI request ID; for a pending partial action, recovery_operation_id is that ORIGINAL pending operation ID.',
