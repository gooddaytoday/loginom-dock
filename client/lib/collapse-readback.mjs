const need=(v,m)=>{if(!v)throw Error('Collapse readback: '+m);};
const pick=(v,keys)=>Object.fromEntries(keys.map(k=>[k,v[k]]));
export function collapseConfigurationReadback({node,phases,operation_id}){
 ...
 need(c?.verified&&c.inventory_complete&&owner(c.node_context)&&c.transposed?.length,'collapse configuration owner');
 need(configured.value.validation?.status==='accepted_by_loginom_next'&&owner(configured.value.validation.node_context),'collapse validation');
 need(saved.value.settings_applied===true&&saved.value.mode==='done'&&owner(saved.value.node_context)
  &&finished.value.settings_applied===true&&['done','execute'].includes(finished.value.mode)&&owner(finished.value.node_context),'collapse finish');
 need(m?.verified&&m.inventory_complete&&m.source_identity_verified
  &&owner(m.node_context)&&m.node_context.output_port?.port===0&&mapped.value.finish?.settings_applied===true&&owner(mapped.value.finish.node_context),'derived output owner');
 need(im?.verified&&im.inventory_complete&&owner(im.node_context)&&im.node_context.input_port?.port===0&&input.value.finish?.settings_applied===true&&owner(input.value.finish.node_context),'input owner');
 ...
 return {kind:'collapse',scope:'observed_before_verified_finish',values_are:'observed_ui_values',node:structuredClone(node),receipt_ids:[input,configured,saved,mapped,finished].map(p=>p.receipt_id),mode:'unpivot',
  information:c.information.map(f=>pick(f,['name','label','type','order'])),transposed:c.transposed.map(f=>pick(f,['name','label','type','order'])),ignore_empty:c.skip_null.value,input_mapping:{port:0,autosync:im.autosync,fields:inputFields},output_mapping:{port:0,autosync:m.autosync,fields:output},package_persistence_verified:false};
}
