...
Основной checkout: ветка `rl-bench`, HEAD
`dee1d210f91818b6f8d7fb5cfd0454eddcc5044c`. Незакоммиченные пользовательские
файлы не менялись. При фиксации SUT одного этого HEAD недостаточно.

| Подтверждённый факт | Следствие для дизайна |
| --- | --- |
| [Индекс подпланов](../../../docs/plans/loginom-dock/README.md) и актуальные начала handoff/status подтверждают 01–04 в source runtime; 05–10 planned | Начальные кейсы используют открытие/граф, текстовый импорт и Калькулятор в режиме «Выражение». Excel, экспорт отдельным узлом, сортировка, группировка и остальные обработчики не нужны |
| Исторические независимые Hermes-проверки 03 и 04 прошли; production и установленный клиент этой работой не обновлялись | За baseline принимается проверенный перед серией снимок source runtime, а не подразумеваемый публичный release; это разные поставки |
...
| В стандартных путях этого Linux-хоста нет Hermes и установленной конфигурации Dock; активный Node — 20.19.2 | Нельзя объявлять профиль Hermes или pinned browser готовыми на этой машине по старой Mac-приёмке |
...
Runner заранее создаёт `series_id`, полный список девяти слотов и порядок
`A1,B1,C1,A2,B2,C2,A3,B3,C3`. Seed/порядок фиксируются до старта. Первый smoke
