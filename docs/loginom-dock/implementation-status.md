# Исходный кандидат Замены, живая приёмка не завершена — 12 сентября 2026

В изолированном потоке узла11 реализован `transform.replace_columns / exact`:
типизированные правила, чтение local Ext cache, сохранение неперечисленных полей,
выход replace/add и публичный readback. Полный внутренний handler выполнил
три поля и вернул точные 6×11 значений; отдельный новый узел подтвердил флаги
other Null. Независимые raw configuration/output проверки прошли, 12/12
подмен отклонены. Публичная result schema исправлена после выявленного отказа
MCP; свежая проверка этого ответа ещё требуется.

Общий client run: 1388 PASS, 1 SKIP, 9 отказов локальных сокетов песочницы;
14/14 повторных проверок соответствующих четырёх файлов вне ограничения
прошли. После последних двух защит output/readback профильный набор 21/21.
Сохранённая диагностика и оставшиеся cases указаны в
[checkpoint](replacement-progress-2026-09-12.md) и [подплане 11](../plans/loginom-dock/11-replacement.md).

**Не готов к отдельной автономной приёмке.** Остались свежий публичный прогон,
матрица изменений/Close/повтора/потерянного ответа/широких правил/пустого входа
и независимое сохранение–reopen–reexecute. Новый source harness и два HTTPS
probe не подключились к `loginom.duckdns.org:443` (SSL connect timeout).
Hermes, push, merge, production build и обновление установленного клиента не выполнялись.
...
обработчиков 03–10 в принятых режимах. Подпланы 11–16 остаются planned;
сохранённые исследования старого пилота не означают их реализации.

[План трёх потоков](../plans/loginom-dock/three-stream-workflow.md) фиксирует
распределение 11 → 14, 12 → 16, 13 → 15, отдельные чаты/ветки/worktree/пакеты,
...
Подготовка окружений, запуск узлов, live-проверки, слияние и установка не выполнялись.
