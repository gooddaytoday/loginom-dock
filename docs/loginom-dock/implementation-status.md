## Зафиксирован четвёртый вариант: пустоты в потоке — 16 сентября 2026

По запросу пользователя подготовлен отдельный preview на 4177: поток
утолщается вокруг символов, их форма образуется пустым пространством,
основной поток проходит по свободным областям. По последующему уточнению
края символов смягчены, редкие частицы пролетают сквозь них; 8/8 обновлённых
геометрических тестов прошли. В первоначальной версии проверены 15/15 адресных
тестов, широкий и узкий экраны, пауза/повтор и резервный JPEG.
Четвёртый вариант сохранён отдельным локальным коммитом; перед фиксацией
повторно прошли 8/8 геометрических тестов. Публикация не выполнялась.
Первые три сцены сохранены в `2a8dd170`.
[Реализация, проверки и ограничения](landing-voids-2026-09-16.md).

---

## Зафиксированы три альтернативы первого экрана — 16 сентября 2026

После замечаний к квадратным узлам подготовлены отдельные локальные варианты:
скопления данных (4174), знаки из частиц (4175), световые русла (4176).
Вариант 4175 дополнительно доработан по запросу пользователя: символы мягко
проступают внутри непрерывных лент, их частицы уносятся дальше по течению.
Резервный кадр обновлён; повторно проверены 9/9 тестов движка и адаптивный вид.
Все три варианта сохранены одним коммитом с общим шаблоном, промежуточным
workflow, тестами и документацией. Перед фиксацией прошли 19/19 проверок.
Сцены и статичные иллюстрации просмотрены в браузере, проверена адаптивность.
Окончательный выбор для публикации и сама публикация ещё не выполнялись;
v1 сохранена в `0f04f9cb`.
[Реализация, проверки и ограничения](landing-variants-2026-09-16.md).

---

## Первый экран переработан в направленный workflow — 16 сентября 2026

По запросу пользователя замкнутая лента заменена графом из шести узлов:
источники → объединение → расчёт → две ветки результата. Частицы движутся
слева направо; v1 сохранена в `0f04f9cb`. 10/10 адресных тестов прошли,
адаптивный вид проверен в браузере. Новый вариант не опубликован.
[Реализация, проверки и ограничения](landing-workflow-2026-09-16.md).

---

## Первый экран лендинга v1 подготовлен локально — 16 сентября 2026

В `landing/` добавлена пространственная анимация потока данных в фирменной
палитре, адаптация для телефона, пауза/повтор и неподвижный fallback.
8/8 адресных тестов прошли; браузерные проверки композиции и шести вариантов
установки выполнены. Серверная сборка и публикация ещё не выполнялись.
[Дизайн, доказательства и ограничения](landing-hero-2026-09-16.md).
...
## RC6 опубликован — 14 сентября 2026

14типов приняты, ordinary profile и native Hermes @file прошли save/reopen аудит.
Клиент0df6da82, сайтfc606397 опубликованы; Mac-плагины и gateway обновлены.
[Проверки, точные ревизии и оставшиеся ограничения](releases/rc6-release-2026-09-14.md).
...
# Полный порядок обработки узлов сохранён — 13 сентября 2026

По запросу пользователя создана [единая инструкция](../plans/loginom-dock/node-workflow-runbook.md)
для старта и продолжения узла: очередь/подплан, отдельные задача/worktree/ветка,
подготовка конфигурации до создания задачи, общая память, изоляция Loginom,
source pins, ревью/доработки, Hermes, события и отдельное разрешение на слияние.
Ссылки добавлены в AGENTS.md, handoff, индекс подпланов и планы потоков;
устаревшие текущие указания о ручной выдаче и own-worktree памяти исправлены.

Последний подтверждённый checkpoint:11/12 завершили единственную доработку и
ожидают candidate/Hermes;13/14 продолжили разработку после остановки пользователем
и перезапуска приложения. Общий actor-доступ проверен во всех четырёх задачах;
урок12 извлечён в общий Peer и прочитан разработчиком11.
[Подробные подтверждения](memory-routing-activation-2026-09-13.json).
Самостоятельная регистрация будущих задач ещё НЕ реализована: текущий routeHash
зависит от списка папок. Совместимый bootstrap обязателен до следующего узла.
Документирование не запускало разработчиков, ревью, Hermes, merge или выпуск.

Далее сохранена хронология предыдущих этапов; её прежние статусы и правила
применяются только к датированным снимкам, а не заменяют текущую инструкцию.
...
# Четыре потока разработки запущены — 12 сентября 2026
...
# Замена: N11-R1 исправлено и проверено — 13 сентября 2026
...
# Разработка Замены завершена, готова к ревью — 13 сентября 2026

Узел11 `transform.replace_columns / exact` завершён в изолированной ветке.
19 групп прямых проверок PASS,26/26 подмен доказательств обнаружены;
клиент1402 PASS/1 SKIP/0 FAIL. Проверены add/replace, точные типы и Null,
частичное обновление, aliases/exclusion, Done/Close, повтор и потеря MCP-ответа,
24 пары, пустой вход, source bytes и save/reopen/reexecute.
Исправлены сохранение excluded links и прокрутка к Add в длинной таблице.

[Отчёт](../plans/loginom-dock/11-development-audit.md),
[сводка](replacement-development-2026-09-13.json),
[checkpoint](replacement-progress-2026-09-12.md).

**Готов следующий этап — одно отдельное ревью в том же чате.**
Автономная приёмка Hermes Sol/low остаётся обязательной после слота координатора.
...
сохранённые исследования старого пилота не означают их реализации.

[План потоков](../plans/loginom-dock/three-stream-workflow.md) фиксирует
распределение 1→11, 2→12→16, 3→13→15, 4→14, отдельные чаты/ветки/worktree/пакеты,
управление пользователем через координатора и ручную выдачу финальной приёмки.
Реестр и next-wave приведены к фактической готовности Слияния и Объединения.
Созданы отдельные чаты Astra/medium, постоянные worktree и ветки от `a3b419bd`.
Все три разработчика подтвердили в `.dock/stream-preflight.json` source runtime,
раздельные профили, Loginom 7.4.2, аккаунты и каталоги test-2/test-1/test-3,
развёрнутые окна и свои диагностические пакеты. Начато исследование мастеров.
Clipboard suite: 5/5 PASS. Это проверка запуска, не live_verified новых обработчиков.
Четвёртый поток добавлен по отдельному запросу после проверки работы первой тройки.
База14 — `0f085c53`; подтверждены свой source MCP/профиль, Loginom7.4.2,
test-4 и каталог /test-4. Создан отдельный диагностический пакет, checkpoint
допуска и source manifest проверены координатором. Начато исследование мастера.
Захват разговоров в Dock archive не активирован; полная изоляция native hooks
не принята. Hermes, ревью, слияние и установка не выполнялись.

Ниже сохранена история проверок и поставок; её результаты относятся к своим ревизиям.
...
Параллельный пилот остановлен по новому указанию пользователя. Все четыре
субагента остановлены; worktree, исходники и наблюдения сохранены, дополнительный
снимок изменений с SHA находится в `.dock/parallel-pilot-20260911/sequential-checkpoint`.
В `AGENTS.md`, планах и реестре закреплена последовательная работа на паузе.
Последнее указание пользователя разрешает завершить только документацию;
никакие узлы, проверки или новые агенты автоматически не запускаются.
При новом запросе сначала оценить общее исправление возврата к сценарию,
затем подплан09 Слияние. [Checkpoint](resume-checkpoint.md).
Обработчики09/11/13 не приняты; завершённого ревью, Hermes, push или установки нет.
Для трёх явно созданных аккаунтов на VPS собран и staged/readback отдельный
каталог `2026.09.11-parallel-pilot.1-candidate`, без production activation.
Основной клиент и пользовательские настройки сохранены.
[Наработки, ограничения и продолжение](parallel-node-pilot.md).

---

# Пилот трёх потоков начат — 11 сентября 2026 — историческая запись

По запросу пользователя запущены три разработчика Astra/medium
(09 Слияние, 11 Замена, 13 Дата и время) и проверяющий Astra/xhigh.
База `7da6f1f8`, отдельные worktree, исходные MCP и браузерные профили.
На созданных пользователем `test-1/2/3` подтверждены три входа без пароля,
раздельные личные каталоги и черновики. Во всех четырёх сессиях подтверждён
доступ к OpenViking через health/find/read с actor scope исходного проекта.
Начальные ошибки identity и устаревших приватных профилей устранены.
Общий live-сбой show_process_node с активной вкладкой «Файлы» исследуется одним
владельцем; это не готовый fix. Новый каталог путей и финальная приёмка впереди.
Узлы, main и установленный клиент этим запуском пока не обновлены.
[Порядок, ограничения и продолжение](parallel-node-pilot.md).
...
# Следующая группа аналитических узлов спланирована — 11 сентября 2026

Добавлены [подпланы11–16 и общие условия](../plans/loginom-dock/next-wave.md):
Замена, Дубликаты и противоречия, Дата и время, Заполнение пропусков,
Кросс-таблица и Свёртка столбцов. Все имеют статус **planned**;
проверены текущая регистрация обработчиков, Help/E2E и согласованность планов.
Реализация, live QA, Hermes-приёмка и выпуск новых узлов не выполнялись.
Уточнены очередь и актуальные правила обычного Hermes в реестре и основном плане.
Подплан05 впоследствии принят в source runtime; 06/09 (Слияние)/10 сохраняют
planned. Исторические аудиты остальных обработчиков не переопределены.
...
Финальная проверка на Node 24.19.0: 438 PASS, 1 SKIP, 0 FAIL;
`git diff --check` прошёл. Логи: `.dock/add-nodes-v2/client-verified-final.txt`.
...
## 7 сентября 2026 — очищены временные файлы после фиксации наработок

По отдельной просьбе пользователя удалены 11 временных каталогов: два каталога
браузерной диагностики, семь копий индекса для проверок и два каталога этой
диагностики в `/tmp`. В них находились 50 568 файлов общим размером 1 047 498 742
байта. Зависимости клиента сохранены; при удалении переходы по symlink не выполнялись.

Пять штатных файлов `benchmark/locomo/openclaw` восстановлены из HEAD: они
отслеживаются Git и описаны в `benchmark/locomo/README.md`, а удаление не связано
с текущим планом Loginom Dock. Исходное состояние этих удалений осталось в patch
резервной копии `.dock/commit-split-20260907-211544/`.

Резервная копия наработок, логи проверок и `.dock/open-draft-v1/` сохранены:
SHA256 всех 21 228 проверенных файлов совпали до и после очистки. Подробный
локальный manifest — `.dock/commit-split-20260907-211544/cleanup-manifest.json`.
После очистки рабочее дерево было чистым; затем актуализированы эта запись,
handoff, checkpoint и комментарий `.gitignore`. Исполняемые исходники Loginom
Dock не менялись; новая живая приёмка, deploy и установка не выполнялись.
...
идентичность документа/workflow, локальная квитанция до эффекта и восстановление
того же operation ID. Проверены несовместимый build, вход, чужой контекст,
сохранность прежних графов, потеря ответа/документа, deadline и отмена.
Фиксированная пауза 500 мс заменена именованными условиями. Живой UI выявил
ранний доступ к меню после аватара; ожидание начальной страницы устранило assert.
Загрузка ожидается локально; диалог сохранения не подтверждается автоматически.

Через клиентский MCP проверены создание, повтор и возврат к workflow; через
живой UI — несколько пакетов, точный файл, несохранённые изменения и отсутствие
файла. Последний Hermes `20260907-205626-3110e56a` получил только цель подготовить
новый черновик: независимый frozen audit **17/17 PASS**, 3 API calls, Sol/low.
Ранние попытки: `20260907-204024-e668f8de` — 16/17 (auditor ошибочно считал
наблюдение исполнением); `20260907-204245-a5bd46f9` — 12/13 (повтор после ещё
видимой загрузки). Оба результата сохранены; после диагностики внесены исправления
и выполнена новая приёмка. Runtime последней приёмки:
`d35ee7f6f51b1f3dcabb76d1388e9adf4544e9356f423c397a0c9be17a0afaa3`.

Полный клиентский набор: 418 PASS, один предусмотренный skip; Python acceptance:
