...
на `67ea9107`. Единственный конфликт затронул этот журнал; записи обеих
веток и общая история сохранены. Остальные 37 файлов PR и 24 файла базы
проверены на точное совпадение со своими исходными версиями.
На Node 24.19.0 прошли все семь landing test-файлов и четыре адресные
проверки упаковки, включая изолированные раскладки. Это согласование
ветки PR; слияние PR в main и публикация сайта не выполнялись.

---

## Исправлены упаковка тестов лендинга и блокирующий прогрев — 16 сентября 2026

Оба списка клиентской упаковки включили зависимости landing-тестов.
Геометрия и первый невидимый кадр `voids` подготавливаются порциями
с отменой при dispose. Семь landing test-файлов и четыре адресные проверки
упаковки прошли, включая изолированные раскладки на Node 24.19.0.
В браузере проверены переходы и пауза; в последнем замере максимум порции
составил 9,8 мс для геометрии и 7,6 мс для кадра. Два прежних сбоя общего
набора runtime-inventory и отсутствие npm-зависимостей задокументированы.
Серверная сборка и публикация не выполнялись.
[Изменения, проверки и ограничения](landing-cycle-2026-09-16.md).

---

## Потоки переключаются по клику с фэйдом — 16 сентября 2026

На общем лендинге клик по визуализации выбирает следующую сцену по кругу.
Следующий рисунок подготавливается заранее; фэйд занимает 700 мс, очередь
LocalStorage обновляется после успешного перехода. Сохранены пауза, replay,
reduced motion и локальное следование за курсором. В браузере проверены
полный круг кликов, Enter, пауза и продолжение после перезагрузки.
Проверены ширины 1280, 721 и 375 CSS px; все семь landing test-файлов прошли.
Изменение локальное, без серверной сборки и публикации.
[Реализация и проверки](landing-cycle-2026-09-16.md).

---

## Четыре сцены объединены в основной лендинг — 16 сентября 2026

Основной HTML теперь запускает clouds → glyphs → ribbons → voids по кругу,
по одному варианту при каждой загрузке документа. Следующая сцена сохранена
в LocalStorage; пауза/replay не меняют очередь. Общий preview работает на 4173,
фиксированные 4174–4177 сохранены. В браузере проверены полный круг, новая
вкладка того же origin и управление анимацией; новые 13/13 тестов и весь набор
шести landing suites прошли. Сборка на сервере и публикация не выполнялись.
[Реализация и проверки](landing-cycle-2026-09-16.md).

---

## Курсор локально притягивает потоки — 16 сентября 2026

Во всех текущих исходных preview (4173–4177) вращение и общий сдвиг от мыши
заменены мягким локальным притяжением. После остановки или ухода курсора
влияние затухает; ракурс и дальние ветви неподвижны. Сохранены мягкие символы
в voids и редкие проходящие частицы. 40/40 адресных тестов прошли, включая
координаты реальной отрисовки всех пяти сцен и неподвижные порты/подписи
workflow. Изменение пока локальное, без серверной сборки и публикации.
[Реализация и проверки](landing-cursor-2026-09-16.md).
