**2026-09-07 — номер Windows hotfix изменён на 0.1.0-rc.2-fix по запросу пользователя.**

Архивы пересобираются с согласованным внутренним номером. Выпуск/тег rc.5,
созданные этой задачей, будут удалены после проверки замены и смены ссылок сайта.
Живая Windows-приёмка пропущена по явному решению пользователя.

**2026-09-07 — подготовлен hotfix 0.1.0-rc.5 для npm Codex на Windows.**

Основа — опубликованный rc.2, только исправление запуска установщика и
соответствующая диагностика/тесты/версии. Пользователь разрешил выпуск
без живой проверки Windows. Подробности: [windows-agent-install.md](windows-agent-install.md).

# Выполнение плана Loginom Dock

Обновлено: 3 сентября 2026 года. База первоначального этапа: `bbc1f9bab00e8255e8d73b138607126c595dde06`.
...
Текущий результат: клиент `0.1.0-rc.1` опубликован, полные сценарии Codex и Hermes
проверены. Отдельный русскоязычный лендинг доступен на `loginom-dock.duckdns.org`;
Studio перенаправляет пользователей на него. Отложенные проверки перечислены в конце.
...
Её полная контрольная сумма совпала с серверной. Автоматическая отправка будущих
копий на внешнее хранилище не настроена.
...
проверка чистой установки из релиза остаётся отложенной.
...
В описании предварительного выпуска явно указаны отложенные проверки.
...
`git diff --check` прошёл. Отложенные пользователем две задачи сохраняют
свой статус. CI не менялся, копии документов во внешнюю память не публиковались.
...
По решению пользователя от 3 сентября 2026 года на потом отложены:

- Проверка чистой установки из опубликованного релиза на macOS/Linux,
  включая обновление, откат и удаление.
- Автоматическая отправка резервных копий на внешнее хранилище.

Отложенные задачи не считаются выполненными и не входят в текущий объём.
Проверки локальных комплектов и восстановления резервной копии, описанные выше,
сохраняют свой результат; они не подтверждают отложенную проверку релиза.
Проверки опубликованных файлов не заменяют отложенную проверку чистой установки.

## Кандидат Windows x64: 3 сентября

Подготовлен исходный код клиента `0.1.0-rc.2` для Windows 11 x64. Общая логика MCP,
архива, сессий и браузера сохранена; платформенный слой добавляет `node.exe`,
`loginom-dock.cmd`, PowerShell-мастер, защищённые ACL и атомарные pointer-файлы
релизов без требования Developer Mode. Codex hooks получили `commandWindows`,
Hermes выбирает Windows launcher, серверный сборщик формирует ZIP `win32-x64`.

Локально прошли 34 клиентских теста, официальный валидатор Codex plugin и пробная
сборка ZIP из 3946 manifest-файлов. Исправлен ранее известный дефект снимка:
`landing/instructions.mjs` и `landing/release.json` входят в bundle, поэтому тесты
лендинга запускаются из изолированного комплекта. Эти проверки ещё не являются
приёмкой на Windows: серверная сборка, установка, native-регистрация Codex/Hermes,
видимый браузер и реальный сценарий Loginom будут зафиксированы после live-проверки.
