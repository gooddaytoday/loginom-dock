# Подготовка общей памяти для чтения и записи — 13 сентября 2026

Пользователь потребовал общую память Loginom Dock для чтения И записи всеми
разработчиками вместо отдельных баз знаний worktree. Прежнее ограничение только
чтением отменено. [Текущая политика](shared-project-memory.md) обновлена в основном
и четырёх рабочих AGENTS.md. Историческая глобальная Experience-память дополнена
новым решением; точное read-back и адресное retrieval проверены.

Фактический MCP — отдельный адаптер в
`/Users/kartamyshev/Git/openviking/integrations/codex-mcp-adapter/`, hooks —
официальный плагин 0.8.1. Готового точного worktree→shared Peer mapping в них нет.
Подготавливаются и тестируются согласованный адаптер/проектная обёртка hooks;
40/40 локальных проверок прошли; постоянный runtime собран, 15 файлов побайтно
проверены. Активные runtime/config/state пока не переключены. Нужны границы завершённых
этапов, сохранение capture-курсоров и штатное доверие новым определениям hooks.
Периодического опроса нет. Не выдавать эту подготовку за работающую общую запись.

Подготовка конфигурации и порядок безопасной активации: `tools/project-memory/README.md`.
Активные конфигурации, trust records и cursor не менялись.
