...
Узел 14 сохраняет отдельную квитанцию отказа; завершение этого служебного хода
ещё ожидается. Реестр помечен approval_blocked_before_process, слот зарезервирован
за 14 без работающей модели. Для продолжения запрошено прямое подтверждение
