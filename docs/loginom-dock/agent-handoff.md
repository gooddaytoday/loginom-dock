# Общая память: hooks доверены, переключение ожидает перезапуска — 13 сентября 2026

Пользователь подтвердил включение общего чтения и записи. Канонические hooks
установлены в основном `.codex/hooks.json`, generation `20260913.3`; `hooks/list`
подтвердил пять доверенных определений во всех четырёх worktree и основном cwd.
Точные хеши получены штатным API, без bypass и без выдуманных trust records.

Для linked Git worktrees приложение берёт hooks из основного checkout: попытка
локальных hooks/config-state узла12 не сработала и была полностью снята.
Конфиги MCP/плагина всех четырёх разработчиков совпадают с исходными; исходные
OpenViking hooks пока включены. Отдельный runtime готов: 46/46 тестов; 25 прямых
standby-запусков сохранили bytes legacy states и не создали shared state.
Для node12 штатно зарегистрировано доверие точной папке; глобальный Peer не менялся.

Узлы11/12 прошли официальный flush под прежними Peer: курсоры1452/1630, хвост
отправлен, ovSessionId=null. Это не перенос знаний; старые записи не удалялись
и журналы не переотправлялись. Перед миграцией заново проверить отсутствие хвоста.

Узлы13/14 продолжают текущие этапы. Не менять их подключения в процессе работы.
Для загруженных задач не найден штатный per-task MCP reload; глобальный reload
запрещён при активных потоках. CUA не разрешает управление UI самого Codex;
обход не выполнялся. Нужен перезапуск приложения пользователем после окончания
этапов13/14. После перезапуска открыть только координатор, затем по одной
незагруженной задаче установить prepared config, проверить отсутствие original hooks,
перенести cursor под official lock и проверить actual actor find/read + новый capture.
Не архивировать задачи и не удалять worktree. Не запускать следующие этапы до проверки.

Точный порядок, проверки и rollback: [tools/project-memory/README.md](../../tools/project-memory/README.md).
Runtime: `.dock/shared-project-memory/runtime/20260913.3`.
Pending configs и trust/standby receipts: `.dock/shared-project-memory/rollout-20260913.3`.
Политика: [shared-project-memory.md](shared-project-memory.md).
Общая запись разработчиков ещё НЕ активирована; автоприёмка узлов НЕ запускалась.
Периодический мониторинг НЕ создавался. Merge/push/deploy/plugin reinstall не выполнялись.

Узел11 также завершил единственную доработку N11-R1 на `91dee921`. Проверены
сохранённые 13/13 групп, 16/16 отрицательных подмен, 1405 PASS/1 SKIP и runtime pin.
Финальное сохранение отдельно не переоткрывалось; Hermes ещё не запускался.
Повторного ревью не было. Узлы11/12 ожидают подготовленный переход памяти и
обязательную автономную приёмку.

Узел12 завершил единственную доработку. N12-R1 закрыт на code SHA `ce10e177`,
HEAD с документами `7bf88255562c59cd32397be3f68c4ec85cb55b38`.
Завершение turn `01a097ff-e564-7c13-a863-6e6d0c93f04e` подтверждено. Проверены
сохранённые 519 PASS, 11 положительных выходных проверок, 3 согласованные
неверные схемы, отклонённые по содержимому, и неизменность оригинального журнала.
Это адресная проверка закрытия N12-R1, без повторного ревью. Узел ожидает перехода
памяти, собственного candidate URI/SHA и слота Hermes; автономной приёмки ещё нет.

Узел14 сообщил блокер исторического каталога: allowed_roots для сохранения
не включает /test-4. Нужен отдельный immutable node14 candidate; подготовленный
локально `2026.09.13-node14.1-candidate` не опубликован, 21 catalog test заявлен
разработчиком. Блокер принят; другие прямые проверки продолжаются. Сборка/публикация
этого кандидата пока не назначались, production/current/shared client не менять.
