# Три потока запущены, идёт разработка — 12 сентября 2026

Актуальное продолжение: [план и реестр потоков](../plans/loginom-dock/three-stream-workflow.md).
Проверены восемь готовых обработчиков 03–10; для 11/12/13 начата разработка,
14–16 ожидают отдельных команд. Новые обработчики ещё не приняты.
Поток1: 11 → 14; поток2: 12 → 16; поток3: 13 → 15.

Работают три обычные задачи Astra/medium в отдельных постоянных worktree и ветках
от `a3b419bd`; точные ID записаны в реестре. Все три `.dock/stream-preflight.json`
подтверждают source runtime, собственный профиль, вход и личный каталог Loginom
7.4.2, геометрию окна и отдельный диагностический пакет. Начато исследование UI.
Native archive разговоров не активирован; отсутствие всех глобальных hook-запусков
пока не доказано. Clipboard suite: 5/5 PASS. Глобальный reload/переустановку не делать.

Пользователь управляет приёмкой, ревью и следующим узлом из исходного чата.
Разработчики заканчивают этап checkpoint «готов к автономной приёмке».
Hermes-слот не выделен. Объединение и поставка — по отдельному запросу;
старый пилот сохранён. Рабочие чаты не архивировать для обновления окружения.

Ниже — предшествующие результаты и контрольные точки, не команды автопродолжения.
...
# Разработка остановлена, режим последовательный — 11 сентября 2026

Последнее указание пользователя: параллельный пилот прекратить, разработку
не продолжать, восстановить документацию для последовательной работы.
Документация обновлена; дальнейшая реализация, ревью и приёмка ждут нового
запроса. Не запускать субагентов, Hermes, сборку, push или установку по старой очереди.

## Состояние на паузе

- Принятая база репозитория: `7da6f1f8`, клиент с Фильтром строк описан ниже.
...
- Диагностические окна с черновиками оставлены открытыми; сохранение пакетов
  не подтверждено. Старые terminal ID принадлежат сессиям субагентов и из основной
  задачи не доступны. Не предполагать возможность продолжить старый вызов;
  сначала проверить живое состояние и сохранность данных выбранного узла.
...
Только после нового запроса пользователя: проверить Git/живые сессии и точные
pins; оценить непринятое общее исправление; работать над одним узлом за раз
в порядке09 → 10 → 11 → 12 → 13 → 14 → 15 → 16, если пользователь не выберет иной.
Прежнее назначение трёх разработчиков Astra/medium и reviewer Astra/xhigh
историческое и не возобновляется автоматически.

Для каждого узла сохраняется последовательный цикл: Codex live-диагностика,
реализация и проверки, ревью, исправления подтверждённых проблем, финальный
