...
Перед следующим выпуском исправить известную зависимость теста сайта из
`client/test/landing.test.mjs` от неупакованного `landing/` (подробности — в
[памятке](agent-handoff.md)). Клиентские проверки должны запускаться именно из
сформированного комплекта, а не только из полного checkout.
...
5. Проверить GitHub-авторизацию, отправить исходный commit, создать prerelease
   `loginom-dock@<новая-версия>` на этом commit и приложить оба архива, суммы и инструкцию.
