...
### 13.2а. Порядок коротких проверок — согласован 6 сентября 2026

**Стенд тестирования и отладки — изменён пользователем 7 сентября 2026.**
Использовать `http://logi-test-plan.bg.local/app/`, для автоматизации —
`http://logi-test-plan.bg.local/app/?testable=true`. Loginom account задавать
явно: `user`, пароль отсутствует. Вход проверен в живом браузере: появился
аватар, загрузочная маска отсутствует; версия на форме входа — **7.4.2**.
Раздел «Файлы» доступен, каталог `/user` открыт через дерево хранилища.
Путь установлен из UI, не выведен из имени аккаунта. Созданы и повторно открыты
`/user/dock-p3` и `/user/dock-p3/packages`. Run20260907-111942 подтвердил загрузку
CSV и совпадение серверной копии (230 bytes/SHA). Сохранение пакета ещё не принято.
На VPS staged candidate2026.09.07-agent.4-candidate для7.4.2 с разрешённым
корнем сохранения `/user/dock-p3/packages`; production не переключён.

Это заменяет прежний выбор `dev-test.bg.local/staging/app-debug/` и аккаунта
`test` для дальнейшей отладки и приёмки P3. До нового автономного прогона
закрепить новый origin/build в конфигурации и candidate compatibility profile,
проверить существующие capability в Loginom7.4.2 и согласовать allowed roots
каталога с наблюдаемым назначением. Старые manifest/pins для build49202 и
`/test/packages` не считать совместимыми автоматически. Hermes остаётся
openai-codex / gpt-5.6-luna / medium на разрешённом новом подключении ChatGPT.


7 сентября первый short import gate принят: run20260907-113445-24e336d0,
независимый audit53/59, wizard_settings_readback=true. CSV и две полные серии
настроек связаны с одним finish/reopen. Далее Calculator/Group и остальные
P3 gates. Synthetic candidates перед интеграцией сверять с реальным lifecycle:
output port калькулятора отдельный, его нельзя подменять шагом node wizard.

**Размер браузера — уточнение пользователя 7 сентября.** Видимый Chromium
запускать с `--start-maximized`, `viewport:null`: Loginom занимает реальную
доступную площадь окна. Проверено на Mac через createSession → Playwright MCP:
2044×1035 CSS pixels, native windowState=maximized, вместо1280×800.
`--start-fullscreen` отдельно проверен (2048×1152), но обычная отладка использует
развёрнутое окно с панелью браузера. Headless сохраняет1280×800. Geometry/epoch
и completeness guards остаются обязательными: размер зависит от монитора,
разворачивание само по себе не доказывает полноту таблиц.

7 сентября после run123922 добавлен независимый verifier Calculator (реальные
мастера узла и обоих портов разделены); пока проверен fixtures, live PASS не получен.
Остальные пять domain verifier остаются открытыми. Group UI исправлен по живой
диагностике: доступные поля выдаются до заполнения выбранных, пустые summary
строки отделены от data cells. Экспорт повторной compaction сохраняет одну
исходную prepare pair при точном совпадении её копии.217 Python/188 UI checks PASS.
Сценарий приёмки проходит последовательно импорт → Calculator → Group/Reform →
выполнение/сохранение/reopen; законченный readback этапа обязателен перед следующим.

Цель — сокращать время до проверенного результата без снижения критериев качества.

1. После сбоя сначала проверить исправление на сохранённых очищенных evidence.
   Исторический audit не менять; новый диагностический результат хранить отдельно
   с указанием исходного run и текущих исходников проверяющего. Это не новая приёмка.
2. Проверять небольшой проблемный участок отдельной goal-only задачей. Полную
   цепочку запускать после подтверждения этого участка; не подсказывать Hermes
   последовательность инструментов ради прохождения проверки.
3. При повторении той же причины не запускать неизменённый replay. Для UI-сбоя
   сначала воспроизвести шаг в отдельной диагностической сессии Loginom,
   сверить E2E/Help, исправить причину и лишь затем возвращаться к Hermes.
4. После небольшой правки один раз выполнять профильные проверки и необходимые
   проверки зависимого поведения. Полный набор выполнять перед общей приёмкой;
   повторять успешные проверки при новых изменениях, сбоях или конкретном риске.
5. Для восстановления контекста читать актуальный checkpoint и нужные разделы.
   В checkpoint хранить краткое состояние и следующий шаг; историю доказательств
   вести в журнале, не копируя её целиком между документами.

Финальный допуск сохраняется: Hermes самостоятельно создаёт сценарий, полные
типизированные данные совпадают с oracle, пакет сохраняется, открывается и
выполняется повторно с проверкой результатов. Диагностический replay, ручной
успех и локальные тесты не заменяют эту приёмку. Профиль модели, guards свежести,
pending/неопределённого эффекта, идентичности и полноты данных сохраняются.

Практическое уточнение после run215646: сначала устранять стоимость общих
наблюдений. Добавлен scope wizard для bounded roots→current wizard в одном
запросе; модели не выдаётся последовательность сценарных действий. Результат
по-прежнему проверяется независимым auditor-ом. Архивные copies компрессора
проверяются offline без очередного model run. Отсутствующее post-reopen mapping
не считается успешным независимо от исправления экспортёра.

7 сентября пользователь явно разрешил новый аккаунт ChatGPT, добавленный
через Hermes OAuth. Он выбран и проверен read-back; прежнее требование того же
account заменено этим выбором. Provider/model/reasoning и запрет fallback сохранены.

Уточнение после run222609: до replay восстановить отозванную авторизацию
исходного Hermes аккаунта с участием пользователя. Приёмочный launcher запрещает
автоматический CLI credential import/recovery и refresh изолированной копии;
аудит требует неизменность подключения и guard receipt. Наличие нужного имени
provider/model само по себе не доказывает использование согласованной подписки.
После повторного входа продолжить короткий import gate, затем полную P3 цепочку.
...
### 13.3. Выбранное направление: операции настройки узла — 7 сентября 2026

**Уточнение пользователя от 7 сентября: явные ожидания обязательны.**
Каждый переход локальной процедуры ожидает конкретное наблюдаемое условие:
нужный каталог, подтверждённый файл, этап и владелец мастера, применённое значение,
редактор нужного столбца, список вариантов, полная схема/сопоставление, узел после
«Готово». У ожидания есть имя, предельный срок, отмена и диагностический результат
в журнале. Интервал опроса не является доказательством готовности; стабильный DOM
служит только дополнительной проверкой пригодности ссылок. Исчезновение элемента,
который ещё не появился, также не доказывает завершение операции. По неизвестному
поведению сначала сверять Help и реальный UI. «Закрыть» отменяет изменения,
«Готово» сохраняет и закрывает мастер, «Выполнить» сохраняет, закрывает и запускает
узел; завершение выполнения проверяется отдельно.


По итогам исследования затрат пользователь закрепил реализацию вариантом 2.
Hermes выбирает логику сценария, компоненты, связи, параметры и
следующий шаг. Локальный Dock исполняет ограниченную типизированную операцию
настройки одного узла/мастера и возвращает проверенный результат. Сервер
остаётся источником знаний. Это планируемая доработка, не готовая возможность.

Первый участок — текстовый импорт, затем Калькулятор с явными настройками
портов. Начать с повторного изучения проблемного перехода в отдельном живом UI,
сверки E2E/Help и контракта одной операции: входные параметры, разрешённые
изменения, полнота readback, частичный эффект, остановка и cleanup. Использовать
существующие drivers/registry; новые handlers требуют runtime/schema/pins и
проверки совместимости. После первого измеренного результата решать об
расширении. Общий исполнитель плана сценария/recipe DSL остаётся этапом P6.
Объём P0–P9 и первого выпуска этим выбором не сокращается.

**Устранение фиксированных ожиданий адаптера (7 сентября).** В executor-preview
и executor-replay явно задавать `timeouts.settle: 0`: стандартный адаптер иначе
добавляет 500 мс после каждого внутреннего обращения, включая чтение. Ожидания
конкретного результата, общий deadline, epoch/identity/interaction guards и
проверки сохранения остаются обязательными. Убирать паузу через настройку Dock,
без патча зависимости. Проверять через настоящий MCP и отдельно через Hermes;
быстрый прямой Playwright-проход не доказывает настройку транспортного слоя.

**Подтверждённый пилот 7 сентября.** Цель `node-import-roundtrip` прошла
самостоятельную Hermes/Luna/medium приёмку: `20260907-170150-12a754e8`,
независимый frozen audit 54/54. Именованное ожидание заканчивается по явному
условию; постоянное повторение четырёх полных снимков удалено. Только после
«Готово» отдельно подтверждается стабильная инкарнация узла двумя наблюдениями;
...
| Действие | Настройки узла | Мастер | Выполнение узла |
| --- | --- | --- | --- |
| «Закрыть» (`btnClose`, с подтверждением при его появлении) | Изменения не сохраняются | Закрывается | Не запускается |
| «Готово» (`btnDone`) | Сохраняются | Закрывается | Не запускается |
| «Выполнить» (`btnExecute`) | Сохраняются | Закрывается | Узел переходит к выполнению |

Локальная операция должна явно различать отмену, сохранение настроек и сохранение
с запуском. Для обычной настройки пройти необходимые страницы через «Далее» и
завершить мастер кнопкой «Готово». «Изменить»/`btnApply` во вложенном редакторе
применяет его изменения к черновику мастера и не заменяет завершение мастера узла.
Возврат прежних значений после «Закрыть» — штатная отмена, а не ошибка сохранения.
Сохранение настроек узла не означает сохранение пакета на диск.

При выборе «Выполнить» учитывать оба эффекта: применение настроек и запуск.
Переход узла к выполнению не доказывает его успешное завершение: нужны отдельные
execution identity/status/result checks. Потеря ответа не разрешает повторный
нажим «Выполнить» без reconciliation исходного запуска. Проверки операций должны
раздельно покрывать все три способа завершения; требования P3 к сохранению,
открытию пакета и повторному выполнению остаются в силе.

**При любой неуверенности в поведении Loginom сначала читать соответствующий
раздел Help**, затем сверять его с E2E и фактическим UI. Не выводить семантику
сохранения, отмены или выполнения только из исчезновения окна, текста кнопки
или предыдущего опыта. Правило относится и к Codex-диагностике, и к инструкциям
Hermes, и к реализации локальных обработчиков.

Повторное открытие не становится обязательным действием обычного режима после
каждого узла: оно нужно, когда без него нельзя доказать применение требуемых
настроек. Альтернативный readback должен быть проверен на реальном Loginom;
закрытия окна и текста успеха недостаточно. Приёмочные roundtrip P3 сохраняются
полностью; отдельные save/reopen/reexecute пакета ими не заменяются.

На каждом существенном внутреннем шаге сохраняются fresh resolve,
session/document/node/port ownership, epoch guards, durable receipts,
reconciliation неизвестного эффекта и запрет слепого retry. Модели выдаётся
компактный результат и фактически незавершённые проверки; полное evidence
остаётся доступным независимому аудитору. Операция принимает разные данные и
значения пользователя, а не содержит готовое решение приёмочного примера.

**Уточнение пользователя 7 сентября: основная отладка — текущей моделью Codex,
Hermes — итоговый критерий успеха.** Текущая модель этой задачи изучает живой UI,
вносит исправления, отлаживает весь заявленный участок и его независимые
verifiers, выполняет профильные проверки. Повторные Hermes runs не используются
как основной цикл поиска ошибок. Когда отладка завершена, запускается итоговая
самостоятельная проверка Hermes через существующую подписку ChatGPT:
`openai-codex` / `gpt-5.6-luna` / `medium`, без смены модели или fallback.
Критерий успеха — независимый PASS всей заявленной цели по реальным данным и
доказательствам. Для P3 это также настройки/связи, полный результат, сохранение,
повторное открытие и выполнение. Ручной успех Codex, unit tests, exit 0 или
самоотчёт Hermes не заменяют этот допуск. Если приёмка выявляет ошибку, основной
разбор и исправление снова выполняет Codex; следующий Hermes run — после отладки.
Это уточнение заменяет прежний порядок частых диагностических Hermes-прогонов.

Видимый браузер и для отладки, и для итоговой приёмки запускать с
...
Сравнивать время и токены на одинаковом принятом участке, учитывая все попытки;
процент будущей экономии пока не доказан. Выбор направления сам по себе не
возобновляет приостановленные прогоны.

Основание и варианты: [исследование затрат](../loginom-dock/2026-09-07-execution-efficiency-review.md).
...
6 сентября пользователь одобрил ускорение: сначала самостоятельный UI-разбор
оставшейся цепочки P3, затем цельные участки с короткими воспроизводимыми
проверками; полный Hermes replay после готовности связного участка. Разрешена
параллельная работа независимых агентов, если она ускоряет разработку. Закреплять
владение файлами, браузером управляет один исполнитель; критерии приёмки и
полный объём P0–P9 сохраняются.


**Обязательное правило проекта от 6 сентября 2026, по указанию пользователя.**
Перед выбором подхода Codex сам изучает нужный участок реального Loginom Web UI:
доступные действия, последовательность переходов, поведение мастеров и признаки
результата. Сопоставляет наблюдения с E2E и Help, затем составляет подход к
реализации и проверке. Это относится ко всей текущей работе выхода из MVP.
Если Hermes часто ошибается или повторно застревает, Codex самостоятельно
воспроизводит проблемный шаг в Web UI и устанавливает причину, прежде чем
повторять неизменённый прогон. Диагностическая сессия отделена от активного
Hermes-прогона. Ручная проверка не закрывает автономную приёмку: после исправления
её выполняет Hermes на gpt-5.6-luna / medium через существующую подписку ChatGPT.
...
**Уточнение приоритета пользователя от 6 сентября 2026:** главный результат —
агент быстро и без ошибок создаёт сценарии Loginom. Пользователь разрешил
эксперименты и альтернативные подходы. Рассматривать проверяемые кандидаты,
включая изменения способа выдачи наблюдений и взаимодействия с мастерами;
сначала сформулировать гипотезу и ограничения, затем review и отдельный replay.
Разрешение не отменяет UI-first, закреплённый профиль Hermes, data-only границы
и проверку сохранённых данных; успешный эксперимент не закрывает этап плана.

В сравнительных сериях рядом с полнотой saved/reopened data acceptance
фиксировать время от начала задачи до независимо проверенного результата,
число вызовов инструментов, отказов/stale observations, повторов и восстановлений,
а также долю полностью завершённых задач. Незавершённые попытки учитывать
отдельно с временем и причиной остановки, не выдавая быстрый отказ за ускорение
успеха. Сравнивать одинаковые задачи и профиль с зафиксированными runtime pins;
стоимость и задержки анализировать вместе с ошибками и качеством результата.
Ускорение не принимается ценой пропуска readback, потери owner/source identity,
повторного неизвестного эффекта или проверки только части данных.
...
### Уточнение поэтапного продолжения — 7 сентября 2026

После run130446 (52/59, P3 не принят) следующий replay ограничен целью
`calculator-roundtrip`: import с полной контрольной серией после всех правок,
затем входной порт, узел и выходной порт Калькулятора с повторным открытием.
Используются те же строгие независимые gates; промежуточный результат не
считается полной приёмкой P3. Дальше — Group/Reform, выполнение и save/reopen.
Стенд user без пароля, /user/dock-p3 и --start-maximized / viewport:null сохранены.
Каталог agent5 добавляет источник-backed узел «Параметры полей»; staged/readback
на VPS, production не активирован. Подробности и ограничения в checkpoint.

### Пауза по просьбе пользователя — 7 сентября 2026, 14:06 МСК

Calculator-roundtrip `20260907-133241-682396db` завершён: 158 API calls,
frozen audit 52/59, P3 не принят. Активных прогонов нет; автоматическое
продолжение остановлено. Перед следующим replay разобрать пропуски полной
повторной проверки импорта и настроек/портов Calculator по tool evidence и
отдельному живому UI. Причина пропусков ещё не установлена. Порядок этапов
сохранён: import/Calculator → Group/Reform → execution/results → save/reopen.
Private Group/Reform candidates не интегрированы. Источник текущего состояния,
проверок и точных pins — [checkpoint](../loginom-dock/resume-checkpoint.md).
